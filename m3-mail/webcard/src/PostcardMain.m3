...
(* Last modified on Wed Jan 17 17:39:34 PST 1996 by mhb       *)
(*      modified on Wed Feb  1 12:20:17 PST 1995 by kalsow    *)
(*      modified on Wed Oct 12 13:55:37 PDT 1994 by birrell   *)
(*      modified on Thu Mar  3 10:16:42 PST 1994 by mcjones   *)
...
(********* Version ********)
CONST Version = "Version 5.9.4w.2 of November 14, 1995";
...
CONST
  ...
  AppName = "WebCard";           (* for window decoration *)
    ...
    LOCK VBT.mu DO
      ...
      IF cl.compose # NIL THEN
        IF cl.config.composeWindow THEN
          ...
          TextPort.ChangeAllTextPorts(
            cl.compose.ownWindow, cl.config.model);
        ELSE
      ...
      TextPort.ChangeAllTextPorts(cl.fv, cl.config.model);
      cl.config.setFonts(cl.popupFV);
      TextPort.ChangeAllTextPorts(cl.popupFV, cl.config.model);
      cl.config.setFonts(cl.niBrowserFV);
      TextPort.ChangeAllTextPorts(cl.niBrowserFV, cl.config.model);
      IF cl.configFV # NIL THEN
        cl.config.setFonts(cl.configFV);
        TextPort.ChangeAllTextPorts(cl.configFV, cl.config.model);
      END;
      IF cl.helpWindow # NIL THEN
        cl.config.setFonts(cl.helpWindow);
        TextPort.ChangeAllTextPorts(cl.helpWindow, cl.config.model);
      END;
    END;
  END AdoptConfig;
...
PROCEDURE SetTextEditColors (cl: MyClosure; te: TextEditVBT.T) =
  (* LL = VBT.mu *)
  BEGIN
...
PROCEDURE InvokeOpenWebFolder(cl: MyClosure; time: VBT.TimeStamp) =
    (* LL = VBT.mu *)
  BEGIN
    cl.applyOp(Op.OpenWebFolder, time);
  END InvokeOpenWebFolder;
...
PROCEDURE DoWebFolderOpen(cl: MyClosure) =
    (* LL = actions *)
  BEGIN
    SetStatusLine(cl, "Please wait ...", StatusLineStyle.Progress);
    UnixMail.ChangeFolders();
    DoOpenFolder(cl, "--Surf--", FALSE, TRUE);
  END DoWebFolderOpen;
...
PROCEDURE ForkedOp(self: ForkedOpClosure): REFANY =
  ...
  BEGIN
    LOCK actions DO
      ...
      | Op.OpenWebFolder => DoWebFolderOpen(cl);
      ELSE
      END;
      LOCK VBT.mu DO
...
PROCEDURE InitInstance(cl: MyClosure) =
  ...
  VAR
    ...
    initImage := NEW(InitImage).init(pm:=logo, op:=PaintOp.Pair(bg, decFg), bg:=bg);
  BEGIN
    cl.path := path;
