...
IMPORT GridPoint, SimpleStep, GridPointCube;
IMPORT Filled;
IMPORT LocPQ;
IMPORT Loc, GridPointList, GridPointSetRoutines AS GridPointSet, GridPointSeq;
IMPORT GridPointSetDef;
IMPORT Cost;
IMPORT Debug, Fmt, Process;
IMPORT RipUp;
IMPORT RouteState;
IMPORT TextRefTbl;
IMPORT IntPair;
...
CONST MaxPoints = 1 * 1000 * 1000;(* max # of points allowed in any cost tbl *)
...
PROCEDURE Commit(rte : T; actual : GridPointList.T) =
  BEGIN
    rte.gpl := actual;
    IF debugAll THEN
      VAR
...
PROCEDURE GetCost(t : T) : CARDINAL = BEGIN RETURN t.finalCost END GetCost;

PROCEDURE RipUpStuff(t : T) : GridPointSet.T =
  BEGIN RETURN t.graveyard END RipUpStuff;

PROCEDURE AddAttribute(self : T; aName : TEXT; attr : REFANY) =
  VAR
    x : BOOLEAN;
  BEGIN
...
PROCEDURE GetAttribute(self : T; aName : TEXT) : REFANY =
  VAR
    attr : REFANY;
  BEGIN
...
PROCEDURE DeleteAttribute(self : T; aName : TEXT) =
  VAR
    attr : REFANY;
  BEGIN
...
PROCEDURE TGridPointList(self : T) : GridPointList.T =
...
PROCEDURE RouteIter(t : T;
                    ...
                    VAR neighs : SimpleStep.Iterator;
  ...
  BEGIN
    ...
    SimpleStep.Neighbors(next.point, to, neighs);
        ...
        BEGIN
         ...
         and (3) the newDistance is less than infinity
          ...
          IF NOT hadIt AND 
            ...
            newDistance <= absoluteMaxCost AND 
            (NOT respectAbsMaxBBox OR GridPointCube.Inside(absoluteMaxBBox, cand.p)) THEN
...
PROCEDURE RouteDriver(rte : T;
                      ...
                      ripupSeq : GridPointSeq.T) RAISES{ Route.NotFound } =
  CONST 
    decay = 1.0d-2;
  VAR
    destPt : GridPoint.T;
    ...
    absoluteMaxCost := MIN(Cost.MaxMaxCost, 
                           ...
                           Cost.BaseCost);
    neighs : SimpleStep.Iterator := NIL;
  ...
  BEGIN
    ...
    TRY LOOP
      ...
      IF RouteIter(rte,q1,to,nonObstacles, count,totCost,
                   finalState,absoluteMaxCost, toSet, destPt,  
                   neighs, boundingBox, keepOuts, 
      ...
      IF iters MOD 20 = 0 THEN 
        ...
        IF RouteIter(rte,q2,from,nonObstacles,count,totCost,
                     finalState,absoluteMaxCost, fromSet, destPt, neighs, boundingBox, keepOuts,                    absoluteMaxBBox, respectAbsMaxBBox, recycleBin) THEN
    ...
    END EXCEPT 
      LocPQ.Empty => 
        ...
        RAISE Route.NotFound("No route exists") 
    |
      Cost.OutOfMemory =>
        Debug.Out("RAISING NotFound -- No route exists (out of memory)");
        RAISE Route.NotFound("No route exists (out of memory)") 
    END; 
    ...
    velocity.v := velocity.v - 
      velocity.v * decay + 
      (1.0d0-FLOAT(finalState.numRipUps(),LONGREAL)) * decay;
    INC(velocity.iter);

    (* remember ripups *)
    IF ripupSeq # NIL THEN
      VAR
    ...
    IF debugRip THEN
      ...
      IF ripWr = NIL THEN
        TRY
          ripWr := FileWr.Open(ripFn)
        EXCEPT
          OSError.E => 
            Process.Crash("Unable to open ripups file \"" & ripFn & "\"!")
        END
      END;

      Wr.PutText(ripWr, "v " & Fmt.Int(velocity.iter) & " " & Fmt.LongReal(velocity.v) & "\n");
...
PROCEDURE RouteSets(self : T; fromSet, toSet : GridPointSet.T; 
                    nonObstacles : GridPointSet.T;
                    multiplier : REAL;          
  ...
  BEGIN 
    LOCK mu DO
      ...
      BEGIN
        ...
        RouteDriver(self, fromSet, toSet, nonObstacles, multiplier, keepOuts, BBox, respectBBox, velocity, ripupSeq);

        IF costHWMWr # NIL THEN
          Wr.PutText(costHWMWr, Fmt.Int(d) & " " & 
            Fmt.Int(Cost.costRecSizeHWM) & " " & GridPoint.Format(from) & 
            " " & GridPoint.Format(to) & "\n");
          Wr.Flush(costHWMWr)
        END
      END;
...
PROCEDURE MarkPointList(rte : T; gplp : GridPointList.T; 
                        nonObstacles : GridPointSet.T) =
  VAR
    text := "";
...
PROCEDURE SetRipUpFilename(fn : TEXT) =
  BEGIN ripFn := fn END SetRipUpFilename;
...
BEGIN 
  IF debugRip THEN
    TRY 
      costHWMWr := FileWr.Open("cost.hwm")
    EXCEPT
      OSError.E => (* skip *)
    END
  END
