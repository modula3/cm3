/* newuob.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int newuob_(integer *n, integer *npt, doublereal *x, 
	...
	doublereal *d__, doublereal *vlag, doublereal *w, doublereal *f, 
	integer *info, doublereal *ftarget, D_fp calfun, integer *itag)
{
    ...
    /* Local variables */
    static doublereal almost_infinity__;
    static integer i__, j, k, ih, nf, nh, ip, jp;
    static doublereal dx;
    static integer np, ix, nfm;
...
/* CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC */
    *f = (*calfun)(n, &x[1], &itag[1]);
/* CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC */
    if (*f != *f || *f > almost_infinity__) {
	if (nf == 1) {
	    fopt = *f;
	    i__2 = *n;
	    for (ix = 1; ix <= i__2; ++ix) {
...
/* Computing 2nd power */
	...
	if (distsq > rhosq) {
	    temp = temp * (distsq / rhosq) * (distsq / rhosq) * (distsq / 
		    rhosq);
	}
