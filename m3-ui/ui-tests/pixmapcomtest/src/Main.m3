...
(* All rights reserved.                                                      *)
(*                                                                           *)
(* Last modified on Thu Oct 17 14:26:14 PDT 1996 by najork                   *)
...
PROCEDURE Repaint (v: Leaf; <*UNUSED*> READONLY rgn: Region.T) =
  ...
  BEGIN
    ...
    (* Paint the pixmap *)
    VBT.PaintPixmap (v, v.rect, v.op, v.src, v.delta);
    ...
    (* Paint a blue outline marking the pixmap domain, offset by delta *)
    WITH dom = Rect.Add (PxmpRect, v.delta) DO
      Rect.Factor (Rect.Inset (dom, -1), dom, part, 0, 0);
      part[2] := Rect.Empty;
      VBT.PolyTint (v, part, PaintOp.FromRGB (0.0, 0.0, 1.0));
    END;
