...
PROCEDURE PaintTile (v: VBT.Leaf; op: PaintOp.T; nw: Point.T) =
  BEGIN
    ...
    VBT.PaintText (v,
                   pt := Point.T {nw.h + 40, nw.v + 62},
                   t  := "Painting Text",
                   op := op);
    VBT.PaintTexture (v,
                      Rect.T {nw.h + 20, nw.h + 130, nw.v + 70, nw.v + 80},
                      op,
                      p);
    VBT.PaintTint (v, Rect.T{nw.h + 20, nw.h + 130, nw.v + 85, nw.v + 95}, op);
    WITH path = NEW (Path.T) DO
      Path.MoveTo (path, Point.T {nw.h +  20, nw.v + 100});
      Path.LineTo (path, Point.T {nw.h + 130, nw.v + 100});
      Path.LineTo (path, Point.T {nw.h + 130, nw.v + 110});
      Path.LineTo (path, Point.T {nw.h +  20, nw.v + 110});
      Path.Close (path);
      VBT.Fill (v, Rect.Full, path, op := op);
    END;
    WITH path = NEW (Path.T) DO
      Path.MoveTo (path, Point.T {nw.h +  20, nw.v + 115});
      Path.LineTo (path, Point.T {nw.h + 130, nw.v + 115});
      Path.LineTo (path, Point.T {nw.h + 130, nw.v + 125});
      Path.LineTo (path, Point.T {nw.h +  20, nw.v + 125});
      Path.Close (path);
      VBT.Stroke (v, Rect.Full, path, width := 2, op := op);
    END;
  END PaintTile;
...
PROCEDURE Repaint (v: VBT.Leaf; <*UNUSED*> READONLY bad: Region.T) =
  BEGIN
    ...
    PaintTile (v, PaintOp.BgBg,                   Point.T {  0,  0});
    PaintTile (v, PaintOp.BgFg,                   Point.T {  0,150});
    PaintTile (v, PaintOp.BgTransparent,          Point.T {  0,300});
    PaintTile (v, PaintOp.BgSwap,                 Point.T {  0,450});
    PaintTile (v, PaintOp.FgBg,                   Point.T {150,  0});
    PaintTile (v, PaintOp.FgFg,                   Point.T {150,150});
    PaintTile (v, PaintOp.FgTransparent,          Point.T {150,300});
    PaintTile (v, PaintOp.FgSwap,                 Point.T {150,450});
    PaintTile (v, PaintOp.TransparentBg,          Point.T {300,  0});
    PaintTile (v, PaintOp.TransparentFg,          Point.T {300,150});
    PaintTile (v, PaintOp.TransparentTransparent, Point.T {300,300});
    PaintTile (v, PaintOp.TransparentSwap,        Point.T {300,450});
    PaintTile (v, PaintOp.SwapBg,                 Point.T {450,  0});
    PaintTile (v, PaintOp.SwapFg,                 Point.T {450,150});
    PaintTile (v, PaintOp.SwapTransparent,        Point.T {450,300});
    PaintTile (v, PaintOp.SwapSwap,               Point.T {450,450});
  END Repaint;
