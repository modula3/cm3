...
PROCEDURE Shape (<*UNUSED*> ch: VBT.T;
  ...
  BEGIN
    RETURN VBT.SizeRange{lo := 620, pref := 620, hi := 621}
  END Shape;
...
PROCEDURE PaintTile (v: VBT.Leaf; op: PaintOp.T; nw: Point.T) =
  BEGIN
    ...
    VBT.PaintTexture (v,
                      Rect.T {nw.h + 10, nw.h + 140, nw.v + 10, nw.v + 140},
                      op,
                      p);
  END PaintTile;
...
PROCEDURE Repaint (v: VBT.Leaf; <*UNUSED*> READONLY bad: Region.T) =
  BEGIN
    ...
    VBT.PaintText (v, pt := Point.T{100,10}, 
                   t := "Testing VBT.PaintTexture for the 16 basic PaintOps");
    PaintTile (v, PaintOp.BgBg,                   Point.T { 10, 20});
    PaintTile (v, PaintOp.BgFg,                   Point.T { 10,170});
    PaintTile (v, PaintOp.BgTransparent,          Point.T { 10,320});
    PaintTile (v, PaintOp.BgSwap,                 Point.T { 10,470});
    PaintTile (v, PaintOp.FgBg,                   Point.T {160, 20});
    PaintTile (v, PaintOp.FgFg,                   Point.T {160,170});
    PaintTile (v, PaintOp.FgTransparent,          Point.T {160,320});
    PaintTile (v, PaintOp.FgSwap,                 Point.T {160,470});
    PaintTile (v, PaintOp.TransparentBg,          Point.T {310, 20});
    PaintTile (v, PaintOp.TransparentFg,          Point.T {310,170});
    PaintTile (v, PaintOp.TransparentTransparent, Point.T {310,320});
    PaintTile (v, PaintOp.TransparentSwap,        Point.T {310,470});
    PaintTile (v, PaintOp.SwapBg,                 Point.T {460, 20});
    PaintTile (v, PaintOp.SwapFg,                 Point.T {460,170});
    PaintTile (v, PaintOp.SwapTransparent,        Point.T {460,320});
    PaintTile (v, PaintOp.SwapSwap,               Point.T {460,470});
  END Repaint;
