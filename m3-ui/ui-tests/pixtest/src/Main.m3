...
(* Last modified on Thu Mar 23 12:07:11 PST 1995 by najork *)
...
PROCEDURE Repaint (v: VBT.Leaf; READONLY bad: Region.T) =
  VAR
    nw := Point.Sub(Rect.Middle(VBT.Domain(v)), 
                    Rect.Middle(VBT.PixmapDomain(v, pix)));
  BEGIN
    VBT.PaintTint (v, bad.r, PaintOp.Bg);
    VBT.PaintPixmap(v, bad.r, PaintOp.BgFg, pix, nw);
  END Repaint;


BEGIN
  WITH res = NEW (VBT.Leaf, repaint := Repaint, shape := Shape) DO
    Trestle.Install(res);
    Trestle.AwaitDelete(res);
  END;
END Main.
