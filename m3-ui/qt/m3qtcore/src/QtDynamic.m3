...
PROCEDURE Cleanup_AbstractDynamicSlot(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
...
REVEAL
AbstractDynamicSlot =
AbstractDynamicSlotPublic BRANDED OBJECT
OVERRIDES
...
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
RETURN QtDynamicRaw.AbstractDynamicQObject_qt_metacall(selfAdr, c, id, arguments);
...
result := QtDynamicRaw.AbstractDynamicQObject_emitDynamicSignal(selfAdr, arg2tmp, arguments);
...
result := QtDynamicRaw.AbstractDynamicQObject_connectDynamicSlot(selfAdr, arg2tmp, arg3tmp, arg4tmp);
...
arg2tmp := M3toC.CopyTtoS(signal);
arg4tmp := M3toC.CopyTtoS(slot);
result := QtDynamicRaw.AbstractDynamicQObject_connectDynamicSignal(selfAdr, arg2tmp, arg3tmp, arg4tmp);
...
PROCEDURE Cleanup_AbstractDynamicQObject(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
...
PROCEDURE New_DynamicQObject0 (self:DynamicQObject; fn: CallbackProc;
 obj: ADDRESS;
): DynamicQObject =
VAR
result : ADDRESS;
...
VAR
ret:ADDRESS; result : AbstractDynamicSlot;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
arg2tmp: C.char_star;
BEGIN
...
PROCEDURE Cleanup_DynamicQObject(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
...
REVEAL
DynamicQObject =
DynamicQObjectPublic BRANDED OBJECT
OVERRIDES
...
PROCEDURE New_Slot0 (self:Slot; parent: DynamicQObject;
 fn: CallbackProc;
 obj: ADDRESS;
): Slot =
VAR
...
PROCEDURE Cleanup_Slot(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
VAR obj : Slot := ref;
BEGIN
  Delete_Slot(obj);
 END Cleanup_Slot;
