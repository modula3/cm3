...
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
QtLayoutRaw.QLayout_setContentsMargins(selfAdr, left, top, right, bottom);
...
arg4tmp := right;
arg5tmp := bottom;
QtLayoutRaw.QLayout_getContentsMargins(selfAdr, arg2tmp, arg3tmp, arg4tmp, arg5tmp);
left := arg2tmp;
top := arg3tmp;
right := arg4tmp;
bottom := arg5tmp;
...
BEGIN
RETURN QtLayoutRaw.QLayout_setAlignment(selfAdr, arg2tmp, ORD(alignment));
END QLayout_setAlignment;

PROCEDURE QLayout_setAlignment1 ( self, l: QLayout;
alignment: AlignmentFlag;
): BOOLEAN =
VAR
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
...
BEGIN
RETURN QtLayoutRaw.QLayout_setAlignment1(selfAdr, arg2tmp, ORD(alignment));
...
VAR
ret:INTEGER; result : SizeConstraint;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
PROCEDURE Cleanup_QLayout(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
VAR obj : QLayout := ref;
BEGIN
  Delete_QLayout(obj);
 END Cleanup_QLayout;
