...
PROCEDURE New_QTextEdit0 (self:QTextEdit; parent: QWidget;
): QTextEdit =
VAR
result : ADDRESS;
...
PROCEDURE New_QTextEdit2 (self:QTextEdit; text: TEXT;
 parent: QWidget;
): QTextEdit =
VAR
result : ADDRESS;
...
PROCEDURE New_QTextEdit3 (self:QTextEdit; text: TEXT;
): QTextEdit =
VAR
result : ADDRESS;
qstr_text := NEW(QString).initQString(text);
...
VAR
ret:ADDRESS; result : QTextDocument;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
VAR
ret:ADDRESS; result : QTextCursor;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
VAR
ret:ADDRESS; result : QTextCharFormat;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
VAR
ret:INTEGER; result : AutoFormatting;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
VAR
ret:INTEGER; result : LineWrapMode;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
BEGIN
ret := QtTextEditRaw.QTextEdit_createStandardContextMenu1(selfAdr, arg2tmp);
...
PROCEDURE Cleanup_QTextEdit(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
