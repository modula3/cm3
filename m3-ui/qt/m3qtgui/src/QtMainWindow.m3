...
PROCEDURE New_QMainWindow0 (self:QMainWindow; parent: QWidget;
flags: WindowTypes;
): QMainWindow =
VAR
result : ADDRESS;
...
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
QtMainWindowRaw.QMainWindow_setToolButtonStyle(selfAdr, ORD(toolButtonStyle));
...
VAR
ret:INTEGER; result : DockOptions;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
VAR
ret:INTEGER; result : DockWidgetArea;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
QtMainWindowRaw.QMainWindow_setUnifiedTitleAndToolBarOnMac(selfAdr, set);
...
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
RETURN QtMainWindowRaw.QMainWindow_unifiedTitleAndToolBarOnMac(selfAdr);
...
VAR
ret:INTEGER; result : ToolBarArea;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
arg2tmp :=  LOOPHOLE(toolbar.cxxObj,ADDRESS);
BEGIN
...
arg3tmp :=  LOOPHOLE(dockwidget.cxxObj,ADDRESS);
BEGIN
QtMainWindowRaw.QMainWindow_addDockWidget1(selfAdr, ORD(area), arg3tmp, ORD(orientation));
...
arg3tmp :=  LOOPHOLE(dockwidget.cxxObj,ADDRESS);
BEGIN
QtMainWindowRaw.QMainWindow_splitDockWidget(selfAdr, arg2tmp, arg3tmp, ORD(orientation));
END QMainWindow_splitDockWidget;

PROCEDURE QMainWindow_tabifyDockWidget ( self: QMainWindow;
 first, second: QDockWidget;
) =
VAR
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
...
BEGIN
RETURN QtMainWindowRaw.QMainWindow_restoreState(selfAdr, arg2tmp, version);
...
PROCEDURE Cleanup_QMainWindow(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
