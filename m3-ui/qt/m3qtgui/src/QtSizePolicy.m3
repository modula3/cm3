...
PROCEDURE New_QSizePolicy1 (self:QSizePolicy;horizontal, vertical: Policy;
): QSizePolicy =
VAR
result : ADDRESS;
BEGIN
result := QtSizePolicyRaw.New_QSizePolicy1(ORD(horizontal), ORD(vertical));
...
PROCEDURE New_QSizePolicy2 (self:QSizePolicy;horizontal, vertical: Policy;
type: ControlType;
): QSizePolicy =
VAR
result : ADDRESS;
BEGIN
result := QtSizePolicyRaw.New_QSizePolicy2(ORD(horizontal), ORD(vertical), ORD(type));
...
VAR
ret:INTEGER; result : ControlType;
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
...
selfAdr: ADDRESS := LOOPHOLE(self.cxxObj,ADDRESS);
BEGIN
QtSizePolicyRaw.QSizePolicy_setHorizontalStretch(selfAdr, stretchFactor);
...
PROCEDURE Cleanup_QSizePolicy(<*UNUSED*>READONLY self: WeakRef.T; ref: REFANY) =
...
REVEAL
QSizePolicy =
QSizePolicyPublic BRANDED OBJECT
OVERRIDES
