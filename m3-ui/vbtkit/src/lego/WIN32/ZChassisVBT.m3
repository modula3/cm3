...
IMPORT Axis, BorderedVBT, Filter, FlexVBT, HVSplit,
       MultiClass, Shadow, ShadowedFeedbackVBT, ShadowedVBT,
       Split, StableVBT, SwitchVBT, TextureVBT, VBT,
       VBTClass, ZChildVBT, ZGrowVBT, ZMoveVBT, ZSplit,
       ZSplitUtils;
IMPORT PixmapVBT, PaintOp, VBTKitResources;
...
PROCEDURE CommonInit (v       : T;
  ...
  BEGIN
    ...
    grow := NEW (ZGrowVBT.T).init (NewBtn ("grow.ppm", shadow));
    banner := HVSplit.New (Axis.T.Hor);
    IF closable THEN
      close :=
        NEW (SwitchVBT.T, callback := Close).init (NewBtn ("close.ppm", shadow));
...
PROCEDURE NewBtn (name: TEXT; shadow: Shadow.T): VBT.T =
  VAR
