...
BEGIN
GLUTRaw.FullScreenToggle();
END FullScreenToggle;

PROCEDURE LeaveFullScreen () =
BEGIN
GLUTRaw.LeaveFullScreen();
END LeaveFullScreen;
...
VAR
ret : REF C.int; result : REF INTEGER;
arg2tmp: C.int;
BEGIN
arg2tmp := size;
ret := GLUTRaw.GetModeValues(mode, arg2tmp);
result := LOOPHOLE(ret,REF INTEGER);
size := arg2tmp;
RETURN result;
END GetModeValues;
...
BEGIN
GLUTRaw.MultiEntryFunc(callback);
END MultiEntryFunc;

PROCEDURE MultiButtonFunc ( callback: CallBack5T;
) =
BEGIN
GLUTRaw.MultiButtonFunc(callback);
END MultiButtonFunc;

PROCEDURE MultiMotionFunc ( callback: CallBack3T;
) =
BEGIN
GLUTRaw.MultiMotionFunc(callback);
END MultiMotionFunc;

PROCEDURE MultiPassiveFunc ( callback: CallBack3T;
) =
BEGIN
GLUTRaw.MultiPassiveFunc(callback);
END MultiPassiveFunc;

PROCEDURE InitContextVersion (majorVersion, minorVersion: INTEGER;
) =
BEGIN
GLUTRaw.InitContextVersion(majorVersion, minorVersion);
END InitContextVersion;

PROCEDURE InitContextFlags (flags: INTEGER;
) =
BEGIN
GLUTRaw.InitContextFlags(flags);
END InitContextFlags;

PROCEDURE InitContextProfile (profile: INTEGER;
) =
BEGIN
GLUTRaw.InitContextProfile(profile);
END InitContextProfile;
