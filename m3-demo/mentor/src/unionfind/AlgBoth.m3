...
(*                                                                           *)
(* Last modified on Thu Aug 18 14:19:08 PDT 1994 by heydon                   *)
(*      modified on Mon Jun 13 16:07:45 PDT 1994 by shillner                 *)
    ...
    VAR node1 := nodes[cmd.arg1]; node2 := nodes[cmd.arg2]; BEGIN
      ...
      IF node1 # node2 THEN
        ...
        IF node1.rank < node2.rank THEN
          node1.up := node2;
          UnionFindIE.Unite(alg, node1.id, node2.id, node2.rank)
        ELSIF node1.rank > node2.rank THEN
          node2.up := node1;
          UnionFindIE.Unite(alg, node2.id, node1.id, node1.rank) 
        ELSE
          node2.up := node1;
...
BEGIN
  ZeusPanel.RegisterAlg(New, "Tree, Both Heuristics", "UnionFind")
END AlgBoth.
