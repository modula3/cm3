...
PROCEDURE NewLayout (READONLY layout    : Layout;
  ...
  BEGIN
    ...
    card := result.head[src];
    result.head[src] := result.next[card];

    (* add card to 'dest' *)
    result.next[card] := result.head[dest];
    result.head[dest] := card;

    (* deal with non-uniqueness of layouts by sorting *)
    IF (src_group = Group.Talon) OR (dest_group = Group.Talon) THEN
      SortTalon(result);
    END;
    IF (src_group = Group.Tableau) AND (result.head[src] = NoCard) THEN
      CollapseTableau(result, src);
    END;
    IF (dest_group = Group.Tableau) AND (RankOf[card] = 13) THEN
      MoveKing(result, dest);
    END;
...
TYPE
  HashValue = [0..1024*1024-1];
  HashTable = ARRAY HashValue OF BITS 1 FOR BOOLEAN;

CONST
  HashMask = LAST (HashValue);  (* ASSUME! == 2^k-1 *)
...
PROCEDURE InitHash () =
  BEGIN
    FOR i := FIRST (hash) TO LAST (hash) DO hash[i] := 0; END;
    numLayouts1 := 0;
    numLayouts  := 0;
    hashCnt     := 0;
  END InitHash;

PROCEDURE NewTree (READONLY layout: Layout): Status =
  VAR h1, h2: HashValue;
  BEGIN
    INC(numLayouts);
    INC(numLayouts1);
    ...
    IF hash[h1] AND hash[h2] THEN
      (* we've probably already seen this layout... *)
...
PROCEDURE Hash1 (READONLY x: Layout): HashValue =
  CONST A = 1234567;   B = 23578327;
  VAR sum := B;
  BEGIN
    FOR i := FIRST (x.head) TO LAST (x.head) DO
      sum := Word.Plus (Word.Times (A, sum), x.head[i]);
    END;
    FOR i := FIRST (x.next) TO LAST (x.next) DO
      sum := Word.Plus (Word.Times (A, sum), x.next[i]);
    END;
    RETURN Word.And (sum, HashMask);
  END Hash1;

PROCEDURE Hash2 (READONLY x: Layout): HashValue =
  CONST A = 7654321;   B = 72387532;
  VAR sum := B;
  BEGIN
    FOR i := FIRST (x.head) TO LAST (x.head) DO
      sum := Word.Plus (Word.Times (A, sum), x.head[i]);
...
VAR
  resultArr : REF ARRAY OF Layout := NIL;
  resultInd : CARDINAL;
...
(* If 'layout' is a known layout in the cached solution,
  ...
  BEGIN
    ...
    FOR i := MAX (resultInd-3, 0) TO MIN(resultInd+25, LAST (resultArr^)-1) DO
      WITH x = resultArr[i] DO
        IF layout = x THEN
          resultInd := i + 1;
          RETURN DescribeMove (x, resultArr[resultInd]);
        END;
      END;
    END;
 ...
 *)
PROCEDURE FindChild (READONLY parent : Layout;
                VAR(*IN/OUT*) seq    : CARDINAL;
                   VAR(*OUT*) child  : Layout): Status =
  CONST
    N1 = Last_Talon - First_Tableau + 1;        (* moves to foundation *)
    N2 = N1 + Last_Tableau - First_Tableau + 1; (* moves to talon *)
    N3 = N2 + Last_Talon - First_Tableau + 1;   (* moves to tableau *)

  VAR result: Tree;  status: Status;  loc: Location;
  BEGIN
    WHILE (seq < N1) DO
      loc := seq + First_Tableau;  INC (seq);
      IF parent.head[loc] # NoCard THEN
        status := FindFoundationSpot(parent, loc, child);
        IF (status # Status.Failure) THEN
          seq := N3 + 1;  (* foundation move => no other children are defined *)
    ...
    WHILE (seq < N2) DO
      loc := seq - N1 + First_Tableau;  INC (seq);
      IF parent.head[loc] # NoCard THEN
        status := FindTalonSpot(parent, loc, child);
    ...
    WHILE (seq < N3) DO
      loc := seq - N2 + First_Tableau;   INC (seq);
      IF parent.head[loc] # NoCard THEN
        status := FindTableauSpot(parent, loc, child);
        IF    (status = Status.Failure) THEN (* can't move this card *)
        ELSIF (status = Status.Success) THEN AddToTree(tree, result);
        ELSE                                 RETURN status;
        END;
