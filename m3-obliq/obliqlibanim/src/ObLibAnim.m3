...
TYPE
  ...
  PROCEDURE EvalGraph(self: PackageGraph; opCode: ObLib.OpCode; 
    ...
    BEGIN
      ...
      | GraphCode.SetDoubleClickAction => 
          ...
          ELSE ObValue.BadArgType(1, "graph", self.name, opCode.name, loc);<*ASSERT FALSE*>  END;
          TYPECASE args[2] OF | ObValue.ValFun(node) => fun1:=node;
          ELSE ObValue.BadArgType(2, "procedure", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
      ...
      | GraphCode.MoveVertex => 
          ...
          ELSE ObValue.BadArgType(1, "vertex", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[2] OF | ObValue.ValReal(node) => real1:=node.real;
          ELSE ObValue.BadArgType(2, "real", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[3] OF | ObValue.ValReal(node) => real2:=node.real;
          ELSE ObValue.BadArgType(3, "real", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
      ...
      | GraphCode.VertexSetFont => 
          ...
          ELSE ObValue.BadArgType(2, "font", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          v1.setFont(font1);
          RETURN ObValue.valOk;    
      ...
      | GraphCode.VertexGetPosition => 
          TYPECASE args[1] OF | ValVertex(node) => v1:=node.vertex;
          ELSE ObValue.BadArgType(1, "vertex", self.name, opCode.name,
      ...
      | GraphCode.MoveVertexHiLi => 
          ...
          ELSE ObValue.BadArgType(1, "vertexHiLi", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[2] OF | ValVertex(node) => v1:=node.vertex;
          ELSE ObValue.BadArgType(2, "vertex", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[3] OF | ObValue.ValBool(node) => bool1:=node.bool;
          ELSE ObValue.BadArgType(3, "bool", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          vh1.move(v1, bool1);
          RETURN ObValue.valOk;
      ...
      | GraphCode.MoveEdge => 
          ...
          ELSE ObValue.BadArgType(1, "edge", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[2] OF | ValVertex(node) => v1:=node.vertex;
          ELSE ObValue.BadArgType(2, "vertex", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[3] OF | ValVertex(node) => v2:=node.vertex;
          ELSE ObValue.BadArgType(3, "vertex", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[4] OF | ObValue.ValBool(node) => bool1:=node.bool;
          ELSE ObValue.BadArgType(4, "bool", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
      ...
      | GraphCode.MoveEdgeBezier => 
          ...
          ELSE ObValue.BadArgType(4, "vertex", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[5] OF | ValVertex(node) => v4:=node.vertex;
          ELSE ObValue.BadArgType(5, "vertex", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[6] OF | ObValue.ValBool(node) => bool1:=node.bool;
          ELSE ObValue.BadArgType(6, "bool", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
      ...
      | GraphCode.EdgeSetArrow => 
          ...
          ELSE ObValue.BadArgType(2, "bool", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
      ...
      | GraphCode.NewPolygon => 
          ...
          ELSE ObValue.BadArgType(1, "array", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          size := NUMBER(array1^);
          list := NIL;
          FOR i := 0 TO size-1 DO 
            TYPECASE array1^[(size-1)-i] OF
            | ValVertex(node) => list := RefList.Cons(node.vertex, list);
            ELSE ObValue.BadArgType(1,"array(vertex)",self.name,opCode.name,loc);<*ASSERT FALSE*> 
            END;
          END;  
      ...
      | GraphCode.MovePolygon => 
          ...
          ELSE ObValue.BadArgType(1, "polygon", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[2] OF 
          | ObValue.ValArray(node) => array1:=node.remote.Obtain();
      ...
      | GraphCode.NewFont => 
          ...
          ELSE ObValue.BadArgType(5, "text", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          TYPECASE args[6] OF | ObValue.ValText(node) => text4:=node.text;
          ELSE ObValue.BadArgType(6, "text", self.name, opCode.name, loc); <*ASSERT FALSE*> END;
          font1 := 
      ...
      | GraphCode.Show => 
          ...
          ELSE ObValue.BadArgType(1, "graph", self.name, opCode.name, loc); <*ASSERT FALSE*> 
          END;
          RETURN ObValue.valOk;      
      | GraphCode.Hide => 
          TYPECASE args[1] OF 
    ...
    BEGIN
      TRY
        ...
        | ObValue.ValArray(node) => 
          ...
          | Thread.Alerted =>
              ObValue.RaiseException(ObValue.threadAlerted,
                "on remote array access", self.location);
          END;
        ELSE ObValue.RaiseError(
  ...
  BEGIN
    ...
    ELSE ObValue.BadArgType(argNo, "color or spectrum", name, opName, loc);<*ASSERT FALSE*> 
    END;
  END ExtractColor;
