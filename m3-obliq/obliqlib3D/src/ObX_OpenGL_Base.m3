...
TYPE
  T = ObGraphicsBase.T BRANDED "ObX_OpenGL_Base.T" OBJECT END;


PROCEDURE AddTObj (base : X_OpenGL_Base.T) =
  <* FATAL ObValue.Error, ObValue.Exception *>
  BEGIN
    WITH obj = Obliq.ObjectClone (Obliq.Vals {TProto}),
         raw = NEW (T, what := "<an X_OpenGL_Base.T>", po := base) DO
...
PROCEDURE GetArg (args    : ObValue.ArgArray; 
                  ...
                  package : ObLib.T; 
                  opCode  : ObLib.OpCode; 
                  loc     : SynLocation.T) : X_OpenGL_Base.T 
...
PROCEDURE SetupModule (loader : ObProtoLoader.T) =
  BEGIN
    ...
    loader.load ("X_OpenGL_Base.obl");
    TProto := loader.get ("X`OpenGL`Base_TProto");

    (*** Register the proxy maker ***)
    X_OpenGL_BaseProxy.MkProxyT := AddTObj;
  END SetupModule;
...
PROCEDURE DoEval (self         : Package; 
  ...
  BEGIN
    ...
    | Code.New =>
      TRY
        WITH  title = ObText.GetArg (args, 1, self, opCode, loc),
              ...
              base  = NEW (X_OpenGL_Base.T).init (title, win_x, win_y, 
                                                  win_w, win_h) DO
    ...
    | Code.NewStd =>
      TRY
        WITH base = NEW (X_OpenGL_Base.T).init ("Anim3D Viewer") DO
          RETURN base.proxy.obj;
        END;
      EXCEPT
      | GraphicsBase.Failure =>
...
PROCEDURE Help (self : ObCommand.T; arg : TEXT; <* UNUSED *> data : REFANY) =
  BEGIN
    ObAux.Help (self, arg, pkgname, "X_OpenGL_Base");
  END Help;
