...
(* "Table" is a generic interface defining partial maps that support
   ...
   \index{map!updatable}
   This version supports only open arrays as the key.
*)

GENERIC INTERFACE OpenArrayTable(Key, Value);
(* Where "Key.T" is an open array type, "Value.T" is not an open array type, both
