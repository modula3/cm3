...
@rem call :make-lib-1 "%~f0" odbc32.lib odbc32.txt
@rem call :make-lib-1 "%~f0" winspool.lib winspool.txt
@rem goto :eof
...
@for %%i in (
    ...
    @call :make-lib-1 "%~f0" %%i.lib %%i.txt cl.exe link.exe
...
{
    my $Signature = $Exports{$Function};
    print "Export $Function $Signature\n";
}
