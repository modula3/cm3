...
\eval{
(if (and 'nil
         (file-exists? "pregexp.scm")
         (file-exists? "pregexp.tar.gz"))
    (call-with-input-file "pregexp.scm"
      (lambda (i)
        (display "\\centerline{\\htmlonly Download \\endhtmlonly")
        (display "\\urlh{pregexp.tar.gz}{Version ")
        (display (caddr (read i)))
        (display "}}")
        (newline))))
}
\centerline{\urlh{http://www.ccs.neu.edu/~dorai}{Dorai
Sitaram}}

\bigskip
\else

\LIBCHAPTER{pregexp}{Perl-Style Regular Expressions}
...
\subsection{\rawscm{pregexp}}

\ifx\MzLibManual\Undefined
The procedure \q{pregexp} takes
\else
\scmutilsection{pregexp}{U-regexp}{procedure}
...
\subsection{\rawscm{pregexp-match-positions}}

\ifx\MzLibManual\Undefined
The procedure \q{pregexp-match-positions} takes
\else
\scmutilsectionO{pregexp-match-positions}{regexp text-string}{start end}{procedure}
...
\subsection{\rawscm{pregexp-match}}

\ifx\MzLibManual\Undefined
The procedure \q{pregexp-match} is called
\else
\scmutilsectionO{pregexp-match}{regexp text-string}{start end}{procedure}
...
\subsection{\rawscm{pregexp-split}}

\ifx\MzLibManual\Undefined
The procedure \q{pregexp-split} takes
\else
\scmutilsection{pregexp-split}{regexp text-string}{procedure}
...
\subsection{\rawscm{pregexp-replace}}

\ifx\MzLibManual\Undefined
The procedure \q{pregexp-replace} replaces
\else

\scmutilsection{pregexp-replace}{regexp text-string insert-string}{procedure}
...
\subsection{\rawscm{pregexp-quote}}

\ifx\MzLibManual\Undefined
The procedure \q{pregexp-quote} takes
\else
\scmutilsection{pregexp-quote}{string}{procedure}
...
\else
Friedl's {\em Mastering Regular Expressions}
\fi
...
\def\pregexpbye{\csname bye\endcsname}
\else
\let\pregexpbye\relax
\fi
