...
(defparameter *pregexp-version* 20080412) ;last change
...
(defun pregexp-read-char-list (s i n)
  (let ((r '()))
    (loop
      (when (>= i n) (error 'pregexp-read-char-list
                            "char class ended too soon"))
      (let ((c (char s i))) (incf i)
        (case c
          (#\] (if (null r)
...
(defun pregexp-match-positions (pat str &optional start end)
  ...
  (unless (consp pat)
    (error 'pregexp-match-positions
           "pattern ~s must be compiled or string regexp"
           pat))
