...
context -- from its direct predecessor Tinylisp (DEC SRC, 1989)
through the Lisp Machines to the broader question of how Algol-family
languages can support this architecture on conventional hardware.

### A taste of MScheme

Before diving into the architecture, here is real code from
[`cspc`](https://github.com/IntelLabs/async-toolkit/tree/main/m3utils/m3utils/csp),
...
the corresponding Modula-3 interface files at build time.  The
generated stub files live in the platform-specific build directory
(e.g., `csp/ARM64_DARWIN/FmtSchemeStubs.m3`).
...
a hand-written module that provides the registration framework;
the individual dispatch procedures and call stubs plugged into it
are generated by `sstubgen`).
...
method name (as an Atom), and the Scheme argument list.  The `interp`
parameter is part of the standard signature for all dispatch
procedures and call stubs; it is passed through to the individual
method stubs, which may need it for exception handling or Scheme
callbacks, even when the dispatcher itself does not use it directly:
...
`sstubgen` shares substantial code with the Network Objects stub
generator (`m3-comm/stubgen`).  Both are `M3ToolFrame` extensions that
walk the M3 AST using the M3 Toolkit (m3tk), extract type information
via an `AstToType` module, and generate code from the type signatures.
The network objects `stubgen` generates marshalling and unmarshalling
...
interoperability.  (See section 8 for the provenance of `sstubgen`
and m3tk.)
...
The generated code has two kinds of wrappers: *call stubs* for
interface procedures and *dispatch procedures* (with associated
*method stubs*) for object method calls.

A **call stub** wraps a single M3 procedure.  It receives an
exception handler (`excH`) as an explicit parameter because the
caller (the Scheme evaluator) provides it directly.  The `interp`
parameter is needed to invoke the exception handler via
`SchemeApply` if the M3 procedure raises.  A generated call stub
looks like:
...
A **dispatch procedure** routes method calls by name.  It does not
take `excH` as a separate parameter; instead, the exception handler
is packed into the `args` list (as the third element) by the
Scheme-level `obj-method-wrap` caller, and the dispatcher extracts
it before forwarding to the appropriate method stub.  The method
stubs themselves have the same signature as call stubs, including
the `excH` parameter.
...
**4. Stub execution.**  The call stub -- generated by `sstubgen`
...
back via `ToScheme`, and handles any M3 exceptions.  The `interp`
parameter is needed to invoke the Scheme-level exception handler via
`SchemeApply` if the M3 procedure raises; the `excH` parameter is
the exception handler closure itself:
...
(All generated call stubs follow the `CallStub_<Interface>_<Procedure>`
naming convention; method stubs use `MethodStub_<Type>_<Method>`.)
...
multiple interpreters may share a global environment.  A global
environment is a `SchemeEnvironment.T` -- the root of a chain of
...
`Safe` (synchronized) and `Unsafe` (unsynchronized) variants;
sharing a global environment across threads requires the `Safe`
variant.
...
The concrete type is revealed in `MpzRep.i3`:
...
The `val` field embeds the GMP `mpz_t` storage directly in the M3
heap object.  The implementation extracts its address for C calls:
...
### 3.9 Tinylisp (Modula-2+)

**Core representation**: `REFANY`.  Tinylisp (DEC Systems Research
Center, 1989) is a Lisp implemented in Modula-2+, the SRC dialect of
Modula-2 that added garbage collection, exception handling, `REFANY`,
and `TYPECASE`.  Symbolic expressions are ordinary Modula-2+ ref
types: `Ref.Integer`, `Ref.LongReal`, `Ref.Boolean`, `Ref.Char`,
`Text.T`, `SxSymbol.T`, `List.T`, `Ref.Vector`.  Any application's
opaque-ref type is directly usable as a Tinylisp value -- a `Wire.T`
or `Wr.T` is a first-class s-expression, just as an `Mpz.T` is a
first-class MScheme value.

**Wrapping Modula-2+ objects**: No wrapping needed.  Any traced
reference is already `REFANY` and therefore a valid Tinylisp value.

**Method dispatch**: Via generated stubs.  A *stub compiler*
(`compile_tli`) processes hand-written `.tli` (TinyLisp Interface)
files and generates Modula-2+ stub procedures that dynamically check
argument counts and types, convert between s-expression types and
Modula-2+ value types, and call the corresponding procedures.  A
`.tli` file declares modules, types, exceptions, and procedures in
s-expression syntax:
...
**Values back to Modula-2+**: Scheme values are already `REFANY`.
The stubs narrow them via `TYPECASE` for value types (`INTEGER`,
`CHAR`, `BOOLEAN`, etc.) and `NARROW` for reference types.

**Registration**: The generated stubs register procedures at startup,
similar to MScheme's approach.

**Execution model**: Unlike MScheme's tree-walking interpreter,
...
**Key trade-offs**: Tinylisp achieves the same zero-wrapping,
shared-heap architecture as MScheme -- the underlying mechanism
(`REFANY` + tracing GC + `TYPECASE`) is identical.  The key
difference is in stub generation: Tinylisp requires a hand-written
`.tli` file (essentially a manual transliteration of the Modula-2+
`.def` file with type annotations), while MScheme's `sstubgen` reads
the standard M3 `.i3` interface files directly via m3tk.  Tinylisp is
also faster at the Lisp level (compiled vs. interpreted) but was
designed as a lightweight extension language rather than a full Scheme.
...
**Shared heap** (MScheme, Tinylisp, JScheme, Kawa): The host
...
This is only possible when both languages are GC'd and share a
runtime (M3 + MScheme, Modula-2+ + Tinylisp) or a VM (Java +
JScheme, Java + Kawa).
...
MScheme's `SchemeObject.T = REFANY` and Tinylisp's use of `REFANY`
are the extreme cases of shared heap: it is not that host objects are
*wrapped* as Lisp/Scheme values, but that they literally *are*
Lisp/Scheme values, by type identity.
...
| Tinylisp | `TYPECASE` (M2+ runtime typecheck)     | Stub dispatch (compiled)           |
...
|----------|------------------------|-----------------------------|------------------------------------------------ |
| MScheme  | Yes (`sstubgen`)       | M3 `.i3` interface files    | M3 Toolkit AST parsing + code generation        |
| Tinylisp | Yes (`compile_tli`)    | Hand-written `.tli` files   | Stub compiler generates M2+ stubs               |
...
The MScheme/M3, Tinylisp/M2+, Kawa/JVM, and JScheme/JVM systems share
a critical advantage: since both languages use the same garbage
collector, there is no coordination problem.  A Scheme closure that captures an M3 object keeps it alive
...
understanding it clarifies why the shared-heap architecture is
confined to a small set of languages: C makes garbage collection
nearly impossible to do correctly.  The language
...
per-call cost through the shared `REFANY` heap, and both have stub
generators that reduce setup cost.  MScheme's `sstubgen` reduces setup
cost further by reading standard `.i3` files directly, while Tinylisp's
`compile_tli` requires hand-written `.tli` files.  Tinylisp compensates
with lower per-call overhead at the Lisp level (compiled vs.
interpreted).
...
separates MScheme from its JVM counterparts, and connects it to
a much smaller lineage.
...
The lineage leading to Modula-3 -- Mesa → Cedar → Modula-2+ →
Modula-3 -- is one of the few that does (see section 8.1).
...
such languages exist, and in those that do, the idea has seldom
been pursued.  The one important exception -- Tinylisp (section 3.9)
-- exploits the same mechanism in Modula-2+, the direct ancestor
of Modula-3.

---

## 7. Design Philosophy: Value Types and the Lisp Machine
...
interoperability: Lisp *was* the systems language and no
interoperability boundary existed at all.  But the Lisp Machine
also illustrates the cost of the approach the Algol family avoids.
As Nelson observes in *Systems Programming with Modula-3* (1991):

> At the other extreme are languages like Lisp, ML, Smalltalk, and
> CLU, whose programming models originate from mathematics. [...]
> These languages have beautiful programming models, but they tend
> to be difficult to implement efficiently, because the uniform
> treatment of values in the programming model invites a runtime
> system in which values are uniformly represented by pointers.  If
> the implementer doesn't take steps to avoid it, as simple a
> statement as `n := n + 1` could require an allocation, a method
> lookup, or both.

The Lisp Machine solved this in hardware.  The Symbolics 3600
(Moon, ISCA 1985) and its successor the Ivory (I-Machine) use
tagged words: every machine word carries a data type tag (4 bits
on the 3600, 6 bits on the Ivory) alongside 32 bits of data or
address.  Twenty-two of the Ivory's type codes designate
*immediate* values -- fixnums, single-precision floats, characters,
small ratios -- where the data is encoded directly in the word.
Fixnum arithmetic is a register-to-register operation; `n + 1`
requires no allocation and no GC involvement.  The remaining 33
type codes designate *pointer* values -- lists, arrays, symbols,
...
linked lists.  Hardware-assisted ephemeral garbage collection
reduces the cost of short-lived heap allocations.  Common Lisp's
`dynamic-extent` declaration allows the compiler to stack-allocate
objects known not to escape their enclosing scope.  With all of
these mechanisms, Lisp Machines do handle fixnum-intensive code
efficiently -- but they achieve it through *dedicated hardware*
that conventional machines lack.

Tagged-pointer hardware has a longer history -- the Burroughs
B5000 (1961) and B6500/B6700 (1969) tagged every word for
machine-level integrity -- but the Lisp Machines were the first to
tag by *user-level data type* (fixnum, cons, symbol, closure),
...
approach: instead of solving the problem in hardware, the *language
itself* distinguishes value types from reference types.  In
Modula-3, `INTEGER`, `REAL`, `BOOLEAN`, `CHAR`, records, and fixed
arrays are value types -- stack-allocated, unboxed, and invisible
to the garbage collector.  Only reference types (`REF`, `OBJECT`,
`TEXT`) participate in the traced heap.  This means that most of a
typical M3 systems program -- local variables, loop counters,
parameter passing, record manipulation -- never touches the GC at
all.  The garbage collector runs only for heap-allocated reference
types, and the compiler needs no special optimization or
declarations to achieve this: the type system guarantees it
statically.
...
heap and are managed by the GC, but the M3 code that the Scheme
stubs call into can be dominated by stack-allocated value types.
The boundary between "GC'd scripting world" and "stack-allocated
systems world" is not a marshaling barrier -- it is the ordinary
Modula-3 type system, which `sstubgen` navigates automatically by
generating `ToScheme`/`ToModula` converters that box and unbox
...
issue -- the JVM boxes primitives like `int` to `Integer` -- but
Java's autoboxing covers only a fixed set of primitive types, while
`sstubgen` handles the full M3 type system including user-defined
records, enumerations, subranges, and multidimensional arrays.)

The Lisp Machine achieved glueless interoperability by making Lisp
the only language.  JScheme approximated it for Java on the JVM.
MScheme approximates it for Modula-3 on conventional hardware --
one type alias (`T = REFANY`), a language that distinguishes values
from references, and a code generator that handles the rest.

---

## 8. Historical Lineage

### 8.1 The Mesa/Cedar/Modula-2+/Modula-3 Language Family

The language features that make MScheme and Tinylisp possible --
`REFANY`, precise tracing GC, `TYPECASE` -- trace a specific lineage
through four decades of systems programming language design.

**Mesa** (Xerox PARC, 1970s) was a systems language for the Alto and
later the Dorado.  It compiled to native code and had strong typing,
but no garbage collection and no universal reference type.

**Cedar** (Xerox PARC, early 1980s) extended Mesa with garbage
collection, `REF ANY`, and runtime type information.  Paul Rovner's
1985 technical report documents the addition of these features.  Cedar
had the infrastructure to support an MScheme-like system -- a
universal traced reference type and a precise GC -- but no one built a
full scripting interpreter on top of it.  The debugger could evaluate
expressions in a stopped frame, but that is not the same as a
general-purpose embedded language.

**Modula-2+** (DEC Systems Research Center, mid-1980s) was DEC SRC's
dialect of Modula-2, adding garbage collection, exception handling,
opaque types, `REFANY`, and `TYPECASE` -- the features Cedar had
pioneered, adapted to a Wirth-family language.  It was the
implementation language for SRC's systems infrastructure and the
language in which Tinylisp was written.

**Modula-3** (DEC SRC / Olivetti Research Center, 1989) refined Modula-2+ into a
clean design with a formal specification.  The Modula-3 Report
(Cardelli, Donahue, Glassman, Jordan, Kalsow, and Nelson, 1989)
codified the type system, the `REFANY` mechanism, and the `TYPECASE`
dispatch that make MScheme's architecture possible.

### 8.2 From JScheme to MScheme
...
language provides `REFANY` and GC.

### 8.3 The sstubgen and m3tk Provenance
...
originally at the Olivetti Research Center (ORC) in Menlo Park,
California, and later at DEC SRC, and
described in his 1990 paper "An Extensible Programming Environment
for Modula-3" (ACM SDE 4).  M3tk began life as part of Olivetti's
full Modula-3 compiler; the cm3 compiler (Bill Kalsow's SRC compiler
lineage) does not use m3tk itself, but the Network Objects `stubgen`
does, and `sstubgen` inherits that dependency.  M3tk is organized
into sub-packages (`m3tk-ast`, `m3tk-syn`, `m3tk-sem`, `m3tk-fe`,
etc.) and is used by both `stubgen` and `sstubgen`, as well as by
...
- Levy, H.M.  *Capability-Based Computer Systems.*  Digital Press,
  1984.  Chapter 2 covers the Burroughs B5000 and B6500 tagged
  architectures.
