...
\midrule
Fixnum       & \texttt{SchemeInt.T = REF INTEGER} (cached $[-256..256]$) \\
Bignum       & \texttt{Mpz.T} (GMP arbitrary-precision) \\
Rational     & \texttt{SchemeRational.T} (num/den as \texttt{Mpz.T}) \\
Inexact real & \texttt{SchemeLongReal.T = REF LONGREAL} \\
MPFR float   & \texttt{SchemeMpfr.T} (configurable precision) \\
Complex      & \texttt{SchemeComplex.T} (re/im as \texttt{SchemeObject.T}) \\
