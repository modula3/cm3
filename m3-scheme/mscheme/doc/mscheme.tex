...
\midrule
Number       & \texttt{SchemeLongReal.T = REF LONGREAL} \\
BigInt       & \texttt{BigInt.T} (from \texttt{cit\_util}) \\
...
\midrule
\texttt{INTEGER}     & \texttt{SchemeLongReal.T} & \texttt{FromI} / \texttt{Int} with range check \\
\texttt{CARDINAL}    & \texttt{SchemeLongReal.T} & \texttt{FromI} / \texttt{Int} with bounds check \\
\texttt{LONGINT}     & \texttt{SchemeLongReal.T} & \texttt{FromI} / \texttt{Int} \\
\texttt{REAL}        & \texttt{SchemeLongReal.T} & \texttt{FromLR(FLOAT(x, LONGREAL))} \\
\texttt{LONGREAL}    & \texttt{SchemeLongReal.T} & \texttt{FromLR} / \texttt{FromO} \\
\texttt{EXTENDED}    & \texttt{SchemeLongReal.T} & \texttt{FromLR(FLOAT(x, LONGREAL))} \\
...
Enumeration          & \texttt{SchemeLongReal.T} & As ordinal index (0, 1, 2, \ldots) \\
Subrange             & \texttt{SchemeLongReal.T} & As base type, with bounds check \\
\bottomrule
\end{tabular}
...
All six numeric types collapse to a single Scheme type (\texttt{REF
LONGREAL}).  Common values (0.0, 1.0, $-$1.0, 2.0) are cached to
reduce allocation.  The \texttt{SchemeChar} module preallocates all
