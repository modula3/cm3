...
(* Sampling profiler for MScheme.
   ...
   The eval loop writes currentProcName on each procedure call.
   A background thread periodically samples it and tallies counts.

   Hot-path cost: one boolean test + one pointer write per call. *)
...
PROCEDURE Results() : TextIntTbl.T;
  (* Return the current sample table (name -> count).
