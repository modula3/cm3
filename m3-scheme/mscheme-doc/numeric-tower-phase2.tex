...
\item \textbf{EXTENDED}: same conversion path as {\tt REAL}
  ...
  \emph{Note:} as of CM3 d5.12, {\tt EXTENDED} is IEEE~754
  binary128 (128-bit quadruple precision) on platforms that
  support it, no longer identical to {\tt LONGREAL}.
  The {\tt ToScheme\_EXTENDED} direction demotes to {\tt LONGREAL}
  (64-bit), losing precision for values that use the extra bits.
  A future enhancement could use {\tt SchemeMpfr.T} with 113-bit
  precision to represent {\tt EXTENDED} values losslessly.
