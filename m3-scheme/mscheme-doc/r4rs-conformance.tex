...
\item {\tt \#e} (exact) and {\tt \#i} (inexact): accepted (no-op).  Since
  MScheme has no exact/inexact distinction, these are harmless.
...
\begin{itemize}[nosep]
\item {\tt exact->inexact}, {\tt inexact->exact} --- implemented as
  identity functions (correct given MScheme's single-type number
  representation).  Defined in {\tt SchemePrimitives.i3}.
...
\item \textbf{Numeric tower.}  The use of a single {\tt LONGREAL}
  ...
  arithmetic, and no complex numbers.  The previously missing essential
  procedures ({\tt exact->inexact}, {\tt inexact->exact}, 2-arg
  {\tt atan}) have been added, as have the {\tt \#b}/{\tt \#o}/{\tt
    \#x} literal prefixes and combined R4RS number prefixes.  The
  remaining missing procedures ({\tt numerator}, {\tt denominator},
  complex number operations) are non-essential and inherently
  unsupported by the single-type representation.
