...
\item {\tt \#e} (exact) and {\tt \#i} (inexact): accepted.  {\tt \#e}
  produces an exact integer; {\tt \#i} produces an inexact {\tt LONGREAL}.
\item Combined prefixes (e.g., {\tt \#e\#x1F}, {\tt \#x\#e1F}) are
  supported in any order, per R4RS~\S2.
\item {\tt \#b} (binary), {\tt \#o} (octal), {\tt \#x} (hexadecimal):
  supported.  The token following the prefix is parsed as an integer in
...
\paragraph{{\tt case} macro.}
The {\tt case} macro expansion uses {\tt memv} (which calls
{\tt eqv?}) to test clause membership, as specified by R4RS~\S4.2.1.
...
\paragraph{Note.} {\tt eqv?} uses {\tt eq?} identity for numbers of the
same type.  Since {\tt 1} is now exact ({\tt SchemeInt.T}) and {\tt 1.0}
is inexact ({\tt SchemeLongReal.T}), {\tt (eqv?\ 1\ 1.0)} is {\tt \#f},
conforming to R4RS.
...
MScheme implements a full numeric tower:
\begin{itemize}
\item \textbf{Exact integers}: fixnums ({\tt SchemeInt.T = REF INTEGER},
  cached $[-256..256]$) and bignums ({\tt Mpz.T}, GMP
  arbitrary-precision).
\item \textbf{Exact rationals}: {\tt SchemeRational.T} with {\tt Mpz.T}
  numerator and denominator, always in lowest terms.
\item \textbf{Inexact reals}: {\tt SchemeLongReal.T = REF LONGREAL}
  (IEEE~754 double precision).
\item \textbf{Arbitrary-precision floats}: {\tt SchemeMpfr.T} (MPFR
  wrapper with configurable precision).
\item \textbf{Complex numbers}: {\tt SchemeComplex.T} with real and
  imaginary parts as arbitrary Scheme numeric values.
\end{itemize}

\noindent The exact/inexact distinction is correctly maintained:
{\tt exact?} returns {\tt \#t} for integers and rationals, {\tt \#f}
for inexact reals.  Arithmetic follows R4RS contagion rules: exact
operations stay exact, and mixing exact with inexact produces inexact.
...
{\tt gcd}, {\tt lcm} are all present.  The rounding procedures
return exact integer results for exact rational inputs, as
required by R4RS.
...
{\tt string->number} also handles rationals ({\tt "1/3"}), complex
({\tt "3+4i"}), dual numbers, and special floats ({\tt +inf.0},
{\tt -inf.0}, {\tt +nan.0}).  {\tt number->string} produces the
{\tt "n/d"} representation for exact rationals.

\subsection{Present Procedures (Recently Added)}

The following essential procedures are now present:
\begin{itemize}[nosep]
\item {\tt exact->inexact} --- converts exact integers and rationals
  to {\tt LONGREAL}.  {\tt inexact->exact} converts floats to exact
  values: integer-valued floats become exact integers, and non-integer
  floats become exact rationals.
\item 2-arg {\tt (atan $y$ $x$)} --- delegates to {\tt Math.atan2}.
  The 1-arg form is unchanged.
\end{itemize}

\subsection{Recently Implemented (Numeric Tower)}

The following non-essential procedures are now available via the full
numeric tower (exact integers, rationals, inexact reals, complex numbers):
...
\midrule
{\tt numerator} & Non-essential & Via {\tt SchemeRational.T} \\
{\tt denominator} & Non-essential & Via {\tt SchemeRational.T} \\
{\tt rationalize} & Non-essential & Stern-Brocot mediants \\
{\tt make-rectangular} & Non-essential & Via {\tt SchemeComplex.T} \\
{\tt make-polar} & Non-essential & Via {\tt SchemeComplex.T} \\
{\tt real-part} & Non-essential & Via {\tt SchemeComplex.T} \\
{\tt imag-part} & Non-essential & Via {\tt SchemeComplex.T} \\
{\tt magnitude} & Non-essential & Via {\tt SchemeComplex.T} \\
{\tt angle} & Non-essential & Via {\tt SchemeComplex.T} \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent
{\tt string-copy} and {\tt string-fill!} are also present
(defined in {\tt SchemePrimitives.i3}).
...
\noindent
{\tt vector-fill!} is also present (defined in {\tt SchemePrimitives.i3}).
...
\noindent
{\tt with-input-from-file}, {\tt with-output-to-file}, and
{\tt char-ready?}  are present as built-in primitives.  The
{\tt with-*} procedures save and restore {\tt interp.input}/{\tt
  interp.output} in a {\tt TRY\ldots FINALLY} block.
...
\item \textbf{Number representation} (\S6.5).
  Full numeric tower: exact integers ({\tt SchemeInt.T}, {\tt Mpz.T}),
  exact rationals ({\tt SchemeRational.T}), inexact reals
  ({\tt SchemeLongReal.T}), MPFR floats ({\tt SchemeMpfr.T}),
  and complex numbers ({\tt SchemeComplex.T}).

\item \textbf{{\tt exact?}\ semantics} (\S6.5.5).
  Returns {\tt \#t} for exact integers and exact rationals;
  {\tt \#f} for inexact reals, MPFR floats, and complex numbers
  with inexact parts.

\item \textbf{{\tt eqv?}\ on numbers} (\S6.2).
  Uses {\tt eq?} identity for same-type numbers.  Since exact and
  inexact are distinct types, {\tt (eqv?\ 1\ 1.0)} evaluates to
...
\addlinespace
\midrule
\multicolumn{3}{l}{\emph{Partial / limitation}} \\
\addlinespace
...
{\tt exit}                  & Terminate process; optional integer exit code (default~0) \\
...
\addlinespace
\multicolumn{2}{l}{\emph{SRFI-151: Bitwise Operations}} \\
\addlinespace
{\tt bitwise-and}, {\tt bitwise-ior}, {\tt bitwise-xor} & Logical operations on exact integers \\
{\tt bitwise-not}           & Bitwise complement \\
{\tt bitwise-nand}, {\tt bitwise-nor}, {\tt bitwise-eqv} & Composite logical operations \\
{\tt bitwise-andc1}, {\tt bitwise-andc2} & And-complement \\
{\tt bitwise-orc1}, {\tt bitwise-orc2} & Or-complement \\
{\tt arithmetic-shift}      & Left/right shift (negative = right) \\
{\tt integer-length}        & Bit length of integer \\
{\tt bit-set?}, {\tt copy-bit} & Single-bit access and modification \\
{\tt bit-count}             & Population count (number of set bits) \\
{\tt first-set-bit}         & Index of lowest set bit \\
{\tt bit-field}, {\tt bit-field-set} & Extract/replace bit fields \\
{\tt bit-field-rotate}, {\tt bit-field-reverse} & Rotate/reverse bit fields \\
\end{longtable}
...
The remaining gaps lie in two areas:

\begin{enumerate}
\item \textbf{Numeric tower.}  MScheme now implements the full R4RS
  numeric tower: exact integers (fixnum/bignum), exact rationals,
  inexact reals, arbitrary-precision floats (MPFR), and complex
  numbers.  All R4RS numeric procedures are present, including
  {\tt numerator}, {\tt denominator}, {\tt make-rectangular},
  {\tt make-polar}, {\tt real-part}, {\tt imag-part}, {\tt magnitude},
  and {\tt angle}.  Reader syntax for rationals ({\tt 1/3}) and
  complex literals ({\tt 3+4i}) is supported.
...
\noindent
All essential R4RS procedures are now present.  The only non-essential
procedures still missing are {\tt transcript-on}/{\tt transcript-off}
and the rational/complex number operations that are inherently
unsupported by MScheme's number representation.
