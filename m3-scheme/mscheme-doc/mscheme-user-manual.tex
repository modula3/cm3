...
\texttt{LinkCompiledScheme()} &
  Generate a synthetic module that imports all compiled Scheme
  modules in this package, ensuring their \texttt{BEGIN} blocks
  execute at startup (triggering registry registration).
  Must appear after all \texttt{scheme()}/\texttt{Scheme()} calls. \\
\bottomrule
\end{tabular}
\end{center}
...
LinkCompiledScheme ()        % after all scheme() calls
