...
implementation.  The numeric tower has been extended beyond
JScheme's original single {\tt double} type to include exact integers
(fixnum and GMP bignum), exact rationals, inexact reals
({\tt LONGREAL}), arbitrary-precision floats (MPFR), complex numbers,
and dual numbers (for automatic differentiation).
Notable limitations inherited from JScheme include:
...
MScheme implements the full R4RS numeric tower.  Exact integers
(fixnums and GMP bignums), exact rationals, inexact {\tt LONGREAL}
values, MPFR arbitrary-precision floats, complex numbers, and dual
numbers (for automatic differentiation) are all supported.
Arithmetic uses contagion: exact values promote to inexact when mixed,
real values promote to complex when mixed, and any value promotes to
dual when mixed with a dual number.
{\tt exact?} returns {\tt \#t} for integers and
rationals; {\tt inexact?} for floats.  {\tt rational?} is true for
integers and exact rationals.  {\tt complex?} is true for all
numbers (per R4RS); {\tt real?} excludes complex values with nonzero
imaginary part.  {\tt exact->inexact} converts exact values to
{\tt LONGREAL}; {\tt inexact->exact} converts floats to exact values
--- integer-valued floats become exact integers, and non-integer floats
become exact rationals.

Additional numeric operations include: {\tt numerator},
{\tt denominator}, {\tt make-rectangular}, {\tt make-polar},
...
{\tt string->number} handles rationals ({\tt "1/3"}), complex
({\tt "3+4i"}), dual numbers, and special floats ({\tt +inf.0},
{\tt -inf.0}, {\tt +nan.0}).  {\tt number->string} handles rationals,
producing the {\tt "n/d"} representation.  {\tt inexact->exact} returns
exact rationals for non-integer floats (e.g., {\tt (inexact->exact 0.1)}
yields {\tt 3602879701896397/36028797018963968}).
{\tt floor}, {\tt ceiling}, {\tt round}, and {\tt truncate} return
exact results for exact rational inputs.  {\tt rationalize} is
implemented using Stern-Brocot mediants.
...
\subsection{Bitwise Operations (SRFI-151)}\label{sec:bitwise}

MScheme implements the SRFI-151 bitwise operations on exact integers
(fixnums and bignums), backed by GMP.  All 18 primitives are available:
...
;; Composite
bitwise-nand  bitwise-nor  bitwise-eqv
...
;; Counting
bit-count  first-set-bit

;; Bit fields
bit-field  bit-field-set  bit-field-rotate  bit-field-reverse
\end{lstlisting}

\noindent
Negative integers use two's complement representation, as required
by SRFI-151.  Examples:

\begin{lstlisting}[language=Scheme]
(bitwise-and #xff #x0f)       ; => 15
(arithmetic-shift 1 64)       ; => 18446744073709551616
(bit-count 255)               ; => 8
(bit-set? 3 #b1010)           ; => #t
(bit-field #b110110 2 5)      ; => 5  (bits 2..4)
\end{lstlisting}
...
MScheme's numeric tower includes exact integers (fixnum and bignum),
exact rationals, inexact {\tt LONGREAL}, MPFR floats, complex numbers,
and dual numbers.
These procedures perform explicit type conversions for Modula-3 interop.

\nopagebreak[4]
\begin{description}
\item[{\tt (number->LONGREAL x)}]
  Convert any numeric value to {\tt LONGREAL}.  Exact integers,
  rationals, and MPFR floats are converted to the nearest
  double-precision value.  Useful as an explicit type annotation
  when passing to stubs that distinguish {\tt REAL} vs.\ {\tt LONGREAL}.
\item[{\tt (number->REAL x)}]
  Convert {\it x\/} to {\tt REAL} (single precision).
  May lose precision.
\item[{\tt (number->EXTENDED x)}]
  Convert {\it x\/} to {\tt EXTENDED} (extended precision on
...
Note: MScheme's numeric tower now includes built-in bignum support
via {\tt Mpz.T}.  Integer arithmetic automatically promotes fixnums
to bignums when results exceed the fixnum range, and standard
arithmetic operators ({\tt +}, {\tt *}, etc.) work transparently on
bignums.  The {\tt BigInt} stub interface described below provides
direct access to the underlying GMP library for applications that
need explicit control over arbitrary-precision integer objects.
...
(* Extract a number -- result may be SchemeInt.T,
   Mpz.T, SchemeLongReal.T, etc. depending on
   the operand types.  Use SchemeNumber to convert. *)
VAR n := SchemeNumber.ToLongReal(result);  (* 6.0d0 *)

(* Bind a value into Scheme *)
scm.bind("my-var", SchemeInt.FromI(42));
...
During program startup, this means:
\begin{itemize}
...
\item The same file may be loaded more than once during startup;
  each load uses the compiled version.
\end{itemize}

Once the REPL starts (\texttt{ReadEvalWriteLoop} or
\texttt{SchemeReadLine.MainLoop}), the compiled registry is
automatically disabled.  Subsequent calls to \texttt{(load
"mathlib.scm")} read the source file from disk, and the new
definitions replace the compiled bindings through the Level~1
binding cells.  This lets the user edit a compiled module and
reload it interactively for debugging without rebuilding the
program.
...
During startup, \texttt{(load "mathlib.scm")} runs the compiled
\texttt{Install} procedure.  Once the REPL is running, the compiled
registry is disabled and \texttt{(load "mathlib.scm")} reads the
source file from disk.  Because the Level~1 compiler routes all
inter-procedure calls through binding cells
(\S\ref{sec:binding-cells}), the new interpreted definitions are
immediately visible to any compiled callers that reference them.

This means:
\begin{itemize}
\item During startup, compiled modules load at native speed.
\item From the REPL, the user can edit a \texttt{.scm} file and
  \texttt{(load ...)} it; the edited definitions take effect
  immediately, even for functions called by other compiled modules.
\item No rebuild is required for interactive debugging.
\end{itemize}

\noindent For example, from the REPL:

\begin{lstlisting}
> (load "mathlib.scm")    ; reads source, replaces compiled bindings
> (set! a 1)              ; mutate a
> a
1
...
\noindent The module is re-executed from the top, all definitions are
re-established, and any mutations made since the previous load are
overwritten.
...
Compiled procedures are visible to the sampling profiler
(Chapter~\ref{ch:profiler}).  Each compiled procedure updates the
profiler's current-procedure and caller registers on entry, so both
the flat profile and the call graph include compiled code alongside
interpreted code.  The overhead is one boolean test and two pointer
writes per call, identical to the interpreted path.

To suppress profiler hooks at compile time (e.g., for maximum
performance in production builds), set the compiler variable before
compilation:

\begin{lstlisting}[language=Scheme]
(set! *profiler-hooks* #f)
\end{lstlisting}

The default is {\tt \#t} (hooks enabled).
