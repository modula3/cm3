...
Files are searched along {\tt **scheme-load-path**}, which is
initialized from {\tt MSCHEMEPATH}.  If the file is not found on disk,
MScheme checks its bundled library (see Chapter~\ref{ch:libraries}).
...
{\tt substring}, {\tt string-append}, {\tt string->list}, {\tt list->string},
{\tt string->symbol}, {\tt symbol->string},
{\tt char?}, {\tt char->integer}, {\tt integer->char},
{\tt vector?}, {\tt make-vector}, {\tt vector-ref}, {\tt vector-set!},
{\tt vector->list}, {\tt list->vector},
...
\item[{\tt (exit)} / {\tt (exit code)}]
  Exit the process.  {\it code\/} defaults to~0.
...
is linked into your application and executes 4--10$\times$ faster than
the interpreter on typical workloads.  The compiler preserves
redefinition semantics: compiled functions look up their callees through
live environment bindings, so \texttt{set!}\ and interpreter-side
\texttt{define} continue to work correctly.
...
(\texttt{scheme.tmpl}) with three entry points:
...
The \texttt{SchemeCompiledRegistry} interface is simple:
...
On ARM64\_DARWIN (Apple M4), the compiler delivers
\textbf{4--10$\times$} speedups on recursive workloads:
...
\midrule
\texttt{(fibonacci 25)}       & 225\,ms    & 2,324\,ms   & 10$\times$ \\
\texttt{(ackermann 3 4)}      & 13\,ms     & 81\,ms      & 6$\times$ \\
\texttt{(loop-sum 10000 0)}   & 36\,ms     & 83\,ms      & 2$\times$ \\
\bottomrule
\end{tabular}
\end{center}

Self-tail-calls benefit particularly from compilation: they are
converted to Modula-3 \texttt{LOOP} statements, eliminating all
call overhead.  The modest measured speedup for \texttt{loop-sum}
reflects \texttt{loadEvalText} parsing overhead in the benchmark
harness, not the compiled loop body itself (which executes in
microseconds).
...
\item \textbf{Redefinition cost.}  Every inter-procedure call goes
  through a binding indirection (\texttt{binding.get()} followed by
  \texttt{NARROW} and virtual dispatch).  This is the cost of
  preserving correct redefinition semantics.  A future Level~2
  compiler could optimize intra-module calls with a fast-path
  pointer comparison.
\end{itemize}
