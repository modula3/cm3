...
dual & \texttt{SchemeDual.T}
     & re/eps for automatic differentiation \\
...
\item \textbf{Numbers} use a full numeric tower: exact integers
  ...
  complex numbers (\texttt{SchemeComplex.T}), and dual numbers
  (\texttt{SchemeDual.T}, for automatic differentiation).  The
...
\item \textbf{Default case} (identifiers and numbers): characters are
  ...
  \begin{itemize}
  \item Parsed as a number if it matches numeric syntax.  The reader
    recognizes integer literals (yielding \texttt{SchemeInt.T} or
    \texttt{Mpz.T}), rational literals such as \texttt{1/3} (yielding
    \texttt{SchemeRational.T}), floating-point literals (yielding
    \texttt{SchemeLongReal.T}), and complex literals such as
    \texttt{3+4i} (yielding \texttt{SchemeComplex.T}).
...
\midrule
\texttt{InstallSandboxPrimitives} & $\sim$190 safe primitives (R4RS core + SRFI-151 bitwise) \\
...
The enumeration \texttt{P} includes entries for the SRFI-151 bitwise
operations (18~primitives total): the six pre-existing operations
(\texttt{bitwise-and}, \texttt{bitwise-ior}, \texttt{bitwise-xor},
\texttt{bitwise-not}, \texttt{arithmetic-shift}, \texttt{integer-length})
and twelve additions (\texttt{bitwise-nand}, \texttt{bitwise-nor},
\texttt{bitwise-eqv}, \texttt{bitwise-andc1}, \texttt{bitwise-andc2},
\texttt{bitwise-orc1}, \texttt{bitwise-orc2}, \texttt{bit-set?},
\texttt{copy-bit}, \texttt{bit-count}, \texttt{first-set-bit},
\texttt{bit-field}, \texttt{bit-field-set}, \texttt{bit-field-rotate},
\texttt{bit-field-reverse}).  All are dispatched to GMP
(\texttt{Mpz.T}) operations after coercing fixnum arguments to bignums.
