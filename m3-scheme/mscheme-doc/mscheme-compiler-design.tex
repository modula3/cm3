...
the test suite, the bugs discovered and fixed during development, the
measured performance results, and a concrete future optimization that
would eliminate virtual dispatch overhead on the fast path.
...
Development of the test suite exposed two compiler bugs.  Both were
subtle interactions between compilation assumptions and Modula-3
scoping semantics.
...
compiled.  Self-compilation is a useful stress test: of the
compiler's 59 top-level definitions, 38 currently compile
successfully and 21 are skipped.
...
\paragraph{Impact.}
One definition in \texttt{compiler.scm} (\texttt{m3-char}) uses
the \texttt{(test => proc)} clause form.
...
The following table summarizes the self-compilation results.
Of 59 top-level definitions in \texttt{compiler.scm}, 38 compile
successfully.  The 21 skipped definitions break down as follows:

\begin{center}
\begin{tabular}{llc}
\toprule
\textbf{Blocking construct} & \textbf{Functions} & \textbf{Count} \\
\midrule
...
Named \texttt{let} only
  & \texttt{iota}, \texttt{read-file} & 2 \\
\texttt{cond =>}
  & \texttt{m3-char} & 1 \\
Rest parameter
  & \texttt{L} & 1 \\
\midrule
\textbf{Total skipped} & & \textbf{21} \\
\bottomrule
\end{tabular}
\end{center}

Adding \texttt{lambda} support would unblock 12 definitions.
Adding both \texttt{lambda} and named \texttt{let} would unblock
19 of the 21, leaving only \texttt{m3-char} (\texttt{cond =>})
and \texttt{L} (rest parameter)---both easy additions that would
complete full self-compilation.  The remaining gaps
(\texttt{letrec}, \texttt{case}, \texttt{do}, internal
...
Steps 1--5 would close all gaps observed in real MScheme programs.
Steps 6--9 are optimizations that become relevant as the compiled
code base grows.
