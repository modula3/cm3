...
The SRFI-151 bitwise operations (\texttt{bitwise-and},
\texttt{arithmetic-shift}, \texttt{bit-field}, etc.---18~primitives
total) are not currently inlined.  Compiled code that calls these
primitives falls back to the interpreter's primitive dispatch, which
delegates to GMP.  Inlining them would require emitting direct
\texttt{Mpz} calls with appropriate fixnum-to-bignum coercion,
which is feasible but has not been prioritized.
...
%----------------------------------------------------------------------
\section{Sampling Profiler Hooks}
\label{sec:opt-profiler}
%----------------------------------------------------------------------

Compiled procedures emit entry hooks for the MScheme sampling profiler
so that compiled code appears in profiling results alongside
interpreted code.  At the entry of each compiled procedure body
(the \texttt{ApplyN\_} procedure for normal functions, or the
\texttt{Apply\_} procedure for rest-parameter functions), the
compiler emits:
...
IF SchemeSamplingProfiler.enabled THEN
  ...
  SchemeSamplingProfiler.currentProcName := "proc-name";
END;
\end{lstlisting}

\noindent
This is the same pattern used in the eval loop for interpreted
procedure calls.  The cost is one boolean test when profiling is
disabled, or one boolean test and two pointer writes when enabled.
No lock is ever taken on the hot path; the sampling thread reads the
registers independently on a timer.

The hook generation is controlled by the \texttt{*profiler-hooks*}
variable (default \texttt{\#t}).  Setting it to \texttt{\#f} before
compilation suppresses the hooks entirely, eliminating even the
boolean test.  In practice the overhead is negligible, and hooks are
enabled by default.
