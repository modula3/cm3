...
BEGIN
  LOOP
    (* (= n 0) *)
    IF SchemeBoolean.TruthO(
         SchemeLongReal.NumEq(n_val, self.k[1])) THEN
      RETURN self.k[0]  (* 1 *)
    ELSE
      (* (* n (factorial (- n 1))) *)
      VAR n_minus_1 := SchemeLongReal.Sub(
                          n_val, self.k[0]);
          (* fetch factorial via Binding -- Level 1 *)
          fact_proc := self.fv[0].get();
      BEGIN
        (* Not a self-tail-call in general, but
           the compiler detects that fv[0] was
           bound to THIS procedure at registration
           time and the call is in tail position,
           so it emits a LOOP instead: *)
        n_val := n_minus_1
        (* and loops *)
      END
    END
