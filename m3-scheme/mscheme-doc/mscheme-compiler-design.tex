...
\midrule
\texttt{(+ a b)}     & \texttt{SchemeLongReal.FromLR(NARROW(a,SchemeLongReal.T)\^{} + \ldots)} \\
...
\texttt{quotient}, \texttt{remainder}) delegate to
\texttt{SchemePrimitive.NumericAdd/Sub/Mul}, which in turn call
\texttt{SchemeNumber.Add/Sub/Mul}.  This handles the full numeric
tower (exact integers, rationals, inexact reals, MPFR, complex)
with correct contagion rules.  The \texttt{number?} predicate
compiles to \texttt{SchemeNumber.Is()}, recognizing all numeric types.
...
\begin{lstlisting}[language=Modula3]
IF SchemeBoolean.TruthO(
     SchemeLongReal.NumEq(a_0, self.k_0)) THEN
\end{lstlisting}
...
and the \texttt{TruthO} type-check dispatch.  The optimization
