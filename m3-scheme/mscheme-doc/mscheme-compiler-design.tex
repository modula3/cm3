...
2,200 lines of Scheme), resides in the \texttt{schemecompiler} package
...
consisting of \textbf{414 test cases}.
All 414 tests pass on ARM64\_DARWIN (Apple Silicon).
...
run on ARM64\_DARWIN (Apple M4, macOS).
...
\midrule
\texttt{(fibonacci 25)}       & 148 ms    & 672 ms   & 4.5$\times$ \\
\texttt{(fibonacci 30)}       & 1,635 ms  & 7,156 ms  & 4.4$\times$ \\
\texttt{(loop-sum 10000 0)}   & 6 ms      & 32 ms      & 5.5$\times$ \\
\texttt{(ackermann 3 4)}      & 6 ms      & 28 ms      & 4.4$\times$ \\
\texttt{(lambda-map-sum 10000)} & 28 ms   & 142 ms     & 5.1$\times$ \\
\bottomrule
\end{tabular}
\end{center}

The \texttt{lambda-map-sum} benchmark exercises lambda closures
and higher-order calls (\texttt{map} over a list with an
accumulating lambda).  The 5.1$\times$ speedup shows that the
lambda compilation machinery---closure allocation, free-variable
capture, and the \texttt{apply} method dispatch---delivers
performance comparable to the other benchmarks.
...
\textbf{Benchmark} & \textbf{Compiled} & \textbf{Interpreted} & \textbf{Speedup} \\
\midrule
MiniMIPS (7 processes) & 46.5\,s & 86.5\,s & 1.86$\times$ \\
HelloWorld (1 process) & 0.73\,s & 1.48\,s & 2.0$\times$ \\
\bottomrule
\end{tabular}
\end{center}

The per-process breakdown for MiniMIPS (compiled mode) shows that
\texttt{compile!} dominates, accounting for approximately 88\%\ of
total time:

\begin{center}
\begin{tabular}{lrrrr}
\toprule
\textbf{Process} & \textbf{loaddata} & \textbf{compile} & \textbf{codegen} & \textbf{total} \\
\midrule
MemStage  & 0.02\,s &  1.69\,s & 0.30\,s &  2.01\,s \\
DataMem   & 0.01\,s &  5.92\,s & 0.75\,s &  6.68\,s \\
Decode    & 0.01\,s &  1.76\,s & 0.33\,s &  2.09\,s \\
Fetch     & 0.00\,s &  1.07\,s & 0.09\,s &  1.17\,s \\
Execute   & 0.04\,s & 29.06\,s & 3.70\,s & 32.79\,s \\
RegFile   & 0.00\,s &  0.99\,s & 0.10\,s &  1.10\,s \\
Writeback & 0.00\,s &  0.35\,s & 0.12\,s &  0.47\,s \\
\bottomrule
\end{tabular}
\end{center}

The Execute process dominates the workload (70\%\ of total time),
reflecting the combinational complexity of the ALU.  Comparing the
\texttt{compile!} phase across modes---29.1\,s compiled versus
49.1\,s interpreted---yields a 1.69$\times$ speedup on the hottest
phase.  The \texttt{codegen} phase shows 2.2$\times$ and
\texttt{loaddata} shows 4.3$\times$.

The 1.86$\times$ overall speedup is lower than the 4--5$\times$
seen in micro-benchmarks because:

\begin{itemize}
\item Not all functions are compiled.  Of the $\sim$\,900 unique
  definitions in the CSP compiler's 50 source files, approximately
  60\%\ are compiled natively.  The remainder fall through to the
  interpreter as passthrough definitions---still executed, but at
  interpreter speed.

\item The hottest inner loops (the AST visitor walkers
  \texttt{prepostvisit-stmt}, \texttt{prepostvisit-expr}, and
  their callees) are now compiled, but they call out to both
  compiled and interpreted code through the general
  \texttt{SchemeProcedure.T} dispatch path.  Each cross-boundary
  call incurs the full binding-lookup + NARROW + virtual-dispatch
  overhead (\S\ref{sec:impl-bottleneck}).

\item The \texttt{compile!} phase is dominated by abstract
  interpretation over large ASTs (thousands of nodes per process),
  where per-node overhead accumulates but individual node processing
  involves substantial work beyond the call overhead that compilation
  eliminates.
\end{itemize}

Further speedup from intra-module direct calls
(\S\ref{sec:gap-intra}) would reduce the per-call overhead on the
fast path and is expected to deliver the most significant additional
improvement.
...
Chapter~\ref{ch:implemented}, the compiler now handles all
commonly-used Scheme constructs.  The compiler self-compiles
101 of 102 definitions (only \texttt{form->string} is skipped
due to \texttt{quasiquote}).  Definitions that use unsupported
constructs are silently skipped by the compilability checker
(\S\ref{ch:boundary}), falling back to the interpreter with no
loss of semantics.

This chapter catalogues the remaining gaps---primarily in call
optimization---and describes concrete strategies for closing each
one.
...
The compiler leaves performance on the table in how it dispatches
calls.  The current call sequence for a function \texttt{foo} is:
...
%----------------------------------------------------------------------
\section{Self-Compilation Status}
\label{sec:gaps-selfcomp}
%----------------------------------------------------------------------

The compiler compiles \textbf{101 of 102} definitions in its own
source (\texttt{compiler.scm}).  The single remaining uncompiled
function is \texttt{form->string}, which uses \texttt{quasiquote}.
...
\begin{enumerate}
\item \textbf{Intra-module direct calls} --- moderate difficulty,
  largest expected performance improvement for compiled code.
  A pointer comparison guards a direct call that bypasses both
  \texttt{NARROW} and virtual dispatch on the fast path
  (\S\ref{sec:gap-intra}).

\item \textbf{Primitive inlining} --- moderate, largest potential
  speedup but requires careful design (\S\ref{sec:gap-inline}).
