...
/{CC} -c/s/{CC} -c \(.*\) \$<$/{CC} {DepDir}{Default}.c \1 @SEGMENT_FLAG@ -o {TargDir}{Default}.c.o/
...
/ -c /s/{\([A-Z_]*\)CC}\(.*\) -c \(.*\)"\([^"]*\)"\([-a-z_]*\)\.c/{\1CC}\2 \3"\4"\5.c -o "{o}"\5.c.o/
# Handle the case of a source file that is "{xxx}"dir:file.c.
/ -c /s/{\([A-Z_]*\)CC}\(.*\) -c \(.*\)"\([^"]*\)"\([-a-z_]*\):\([-a-z_]*\)\.c/{\1CC}\2 \3"\4"\5:\6.c -o "{o}"\6.c.o/
