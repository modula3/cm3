  ...
  ( ElemCt : CARDINAL; 
    VAR Open : REF ARRAY OF MetadataRef; 
        (* ^As Rodney Bates has been told by llvm folk, llvm will immediately copy
           everything passed in and take responsibility for memory-managing the copy, 
           so we can let this be GCed, once done giving ArrRef to llvm. *) 
...
IO.Put("subrange debug\n");
    ...
    RETURN M3DIB.DIBgetOrCreateSubrange
...
PROCEDURE DebugProcType(self : U; p : ProcTypeDebug) : M3DIB.LLVMDISubroutineType =
(*  VAR
...
PROCEDURE DebugVar(self : U; v : LvVar; argNum : CARDINAL := 0) =
  ...
  BEGIN
    ...
    lvDebug := M3DIB.DIBcreateLocalVariable(self.debugRef, tag, self.funcRef, LTD(name), self.fileRef, self.curLine, tyVal, FALSE, flags, argNum);
...
PROCEDURE DebugGlobals(self : U; var : LvVar) =
  VAR
    ...
    debugObj : REFANY;
    glob : RecordDebug;
    gep,globAlias : LLVM.ValueRef;
    lVal : M3DIB.LLVMDIGlobalVariable; 
