...
define void @print__print_I3() personality ptr @__gxx_personality_v0 {
entry:
  ret void
}

; RT0.ModuleInfo for print (13 fields, 104 bytes)
%RT0_ModuleInfo_t = type { ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, i64, ptr, i64 }
@print_M3_info = internal global %RT0_ModuleInfo_t {
  ptr null,  ; file (+0)
  ...
  ptr null,  ; imports (+72)
  i64 0,  ; link_state (+80)
  ptr @print_I3,  ; binder (+88)
  i64 3  ; gc_flags (+96)
}

define ptr @print_I3(i64 %mode) personality ptr @__gxx_personality_v0 {
entry:
  %do_body = icmp ne i64 %mode, 0
  br i1 %do_body, label %run, label %done
run:
  call void @print__print_I3()
  br label %done
done:
  ret ptr @print_M3_info
}
