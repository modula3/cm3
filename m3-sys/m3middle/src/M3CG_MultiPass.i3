...
IMPORT M3CG, Target, M3CG_AssertFalse, M3ID;
...
(* These create procs. *)
TYPE import_procedure_t = op_tag_t OBJECT name: Name; n_params: INTEGER; return_type: Type; callingConvention: CallingConvention; return_typeid: TypeUID := 0; return_typename := M3ID.NoID; OVERRIDES replay := replay_import_procedure END;
TYPE declare_procedure_t = op_tag_t OBJECT name: Name; n_params: INTEGER; return_type: Type; level: INTEGER; callingConvention: CallingConvention; exported: BOOLEAN; parent: INTEGER(*proc_t*); return_typeid: TypeUID := 0; return_typename := M3ID.NoID; OVERRIDES replay := replay_declare_procedure END;
...
TYPE start_try_t = op_t OBJECT OVERRIDES replay := replay_start_try END;
TYPE end_try_t = op_t OBJECT OVERRIDES replay := replay_end_try END;
TYPE invoke_direct_t = op_t OBJECT proc: INTEGER(*proc_t*); type: Type; next,handler : Label OVERRIDES replay := replay_invoke_direct END;
TYPE invoke_indirect_t = op_t OBJECT type: Type; callingConvention : CallingConvention; next,handler : Label OVERRIDES replay := replay_invoke_indirect END;
TYPE landing_pad_t = op_t OBJECT type: ZType; handler: Label; catches : REF ARRAY OF TypeUID OVERRIDES replay := replay_landing_pad END;
...
PROCEDURE replay_start_try(self: start_try_t; replay: Replay_t; cg: cg_t);
PROCEDURE replay_end_try(self: end_try_t; replay: Replay_t; cg: cg_t);
PROCEDURE replay_invoke_direct(self: invoke_direct_t; replay: Replay_t; cg: cg_t);
PROCEDURE replay_invoke_indirect(self: invoke_indirect_t; replay: Replay_t; cg: cg_t);
PROCEDURE replay_landing_pad(self: landing_pad_t; replay: Replay_t; cg: cg_t);
