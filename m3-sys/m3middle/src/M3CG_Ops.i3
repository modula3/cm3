...
(* The debugging information for a type is identified by a globally unique
   32-bit _UID_, generated by the front-end.  The following methods generate
...
(* Enumeration type.  The subsequent n_elts occurrences of
   declare_enum_elt give the value names. *) 
...
(* Record type.  The subsequent n_fieldss occurrences of
   declare_field define the value fields. *) 

declare_field (n: Name;  o: BitOffset;  s: BitSize;  t: TypeUID);
(* Field of a record or object type. *) 
...
(* Formal of a procedure *type*. *) 

declare_raises (n: Name);
(* n is qualified, unmangled (i.e., with a dot. *) 
...
(* Object type.  The subsequent n_fields occurrences of declare_field
   define the fields.  The subsequent n_fields occurrences of declare_method
   define the methods.  brand=NIL ==> t is unbranded. *)
...
(* declares an exception in this compilation unit, named 'n' identified
   with the address 'base+offset' that carries an argument of type 'arg_type'.
   If 'raise_proc', then 'base+offset+BYTESIZE(ADDRESS)' is a pointer to the
   procedure that packages the argument and calls the runtime to raise
   the exception. *)
...
(* declare and import the external procedure with name 'n' and 'n_params'
   ...
   by the subsequent 'declare_param' calls.  n is a mangled linker name. *)
...
(* declare a procedure named 'n' with 'n_params' formal parameters
   ...
   The formal parameters are specified by the subsequent n_params
