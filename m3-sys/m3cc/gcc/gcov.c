...
/* ??? This should depend on sizeof (long) for the target when reading the
   .da file.  The current code works only as long as the target and host
   have the same size longs.  */
...
open_files ()
{
  ...
  if (object_directory)
      ...
      if (object_directory[objdir_count - 1] != '/')
	{
      ...
      if (cptr)
	{
	  strcat (d_file_name, cptr + 1);
	  strcat (bb_file_name, cptr + 1);
	  strcat (bbg_file_name, cptr + 1);
	}
      else
	{
	  strcat (d_file_name, input_file_name);
    ...
    {
      strcpy (d_file_name, input_file_name);
...
read_files ()
{
  ...
  if (d_file)
	...
	fprintf (stderr, ".da file contents exhausted too early\n");
      ...
      if (fread (&total, sizeof (total), 1, d_file) != 0)
  ...
  bb_data = (long *) xmalloc (buf.st_size);
...
scan_for_source_files ()
{
  ...
  for (count = 0; count < bb_data_size; count++)
      ...
      if (line_num == -1)
	{
	  ...
	  s_ptr = sources;
	  while (s_ptr && strcmp (s_ptr->name, (char *) ptr))
	    s_ptr = s_ptr->next;
