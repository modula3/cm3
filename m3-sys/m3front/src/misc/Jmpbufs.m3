...
PROCEDURE CheckProcPop (VAR cs: CheckState; proc: Proc) =
  BEGIN
    DebugPrint("CheckProcPop 1 " & IdToText(proc.name) & " try_count:"
                 & Fmt.Int(cs.try_count));
    proc.try_count := cs.try_count;
    IF cs.save # NIL THEN cs := cs.save^; ELSE cs := CheckState{}; END;
    IF cs.proc # NIL THEN
      DebugPrint("CheckProcPop 2 " & IdToText(cs.proc.name) & " try_count:"
                   & Fmt.Int(cs.try_count));
    END;
  END CheckProcPop;
...
PROCEDURE CompileTryGetJmpbuf (VAR try: Try): CG.Var =
  ...
  BEGIN
    IF NOT Target.Alloca_jmpbuf THEN RETURN NIL; END;
    ...
    IF try_index >= NUMBER(try.proc.jmpbufs^) THEN
      Error.Msg("CompileTryGetJmpbuf try_index:" & Fmt.Int(try_index)
                  & " out of bounds:" & Fmt.Int(NUMBER(try.proc.jmpbufs^)));
    END;
...
PROCEDURE CompileProcAllocateJmpbufs (p: Proc) =
  VAR
    ...
    try_count: INTEGER;
  BEGIN
    IF (NOT Target.Alloca_jmpbuf) OR Target.Has_stack_walker THEN
      RETURN;
    END;
    ...
    <* ASSERT try_count >= 0 *>
    CG.Comment(-1, FALSE, "AllocateJmpbufs " & IdToText(p.name)
                            & " try_count:" & Fmt.Int(try_count));
    IF try_count = 0 THEN RETURN; END;

    (* We are going to alloca(size * n), where n is the number of TRYs;
       then each TRY will use its jmp_buf.  As the jmp_buf size is not a
       constant here, multiplication n * jmp_buf is not efficient.
       Therefore, use addition to precompute the n pointers (TRY can occur
       in a loop -- making it more valuable to store the individual
       pointers.) *)

    p.jmpbufs := NEW(REF ARRAY OF CG.Var, try_count);
    FOR i := 0 TO try_count - 1 DO
      p.jmpbufs[i] := CG.Declare_local(M3ID.NoID, Target.Address.size,
                                       Target.Address.align, CG.Type.Addr,
                                       0, in_memory := FALSE,
                                       up_level := FALSE, f := CG.Likely);
    END;

    (* If referencing the size more than once, store the global in a
       local. *)

    IF try_count > 1 THEN
      CG.Load_int(Target.Word.cg_type, size);
      size := CG.Declare_temp(Target.Address.size, Target.Address.align,
                              Target.Word.cg_type, in_memory := FALSE);
      CG.Store_int(Target.Word.cg_type, size);
    END;

    (* jmpbufs[0] = alloca (size * try_count) unless try_count == 1 then
