...
IMPORT LInt, IntegerExpr, ProcType, CheckExpr, Card;
...
PROCEDURE Compile (ce: CallExpr.T) =
  ...
  BEGIN
    ...
    CG.Push (t2);
    CG.Push (t3);
    CG.Insert (Target.Longint.cg_type);
    CG.Free (t2);
    CG.Free (t3);
  END Compile;
...
PROCEDURE Initialize () =
  VAR
    ...
    f1 := Formal.NewBuiltin ("y", 1, LInt.T);
    f2 := Formal.NewBuiltin ("i", 2, Card.T);
    f3 := Formal.NewBuiltin ("n", 3, Card.T);
    t  := ProcType.New (LInt.T, f0, f1, f2, f3);
  BEGIN
    Z := CallExpr.NewMethodList (4, 4, TRUE, TRUE, TRUE, LInt.T,
                                 NIL,
