...
IMPORT LInt, IntegerExpr, ProcType, Host, Card;
...
PROCEDURE Compile (ce: CallExpr.T) =
  ...
  BEGIN
    ...
    ELSIF x1 THEN
      ...
      b := TInt.FromInt (Target.Longint.size - i1, Target.Pre.Integer, max);
      <*ASSERT b*>
      Expr.Compile (ce.args[0]);
      CG.Force ();
      CG.Load_intt (i1);
    ...
    ELSE
      ...
      IF Host.doRangeChk THEN
        b := TInt.FromInt (Target.Longint.size, Target.Pre.Integer, max);
        <*ASSERT b*>
        CG.Push (t1);
        CG.Push (t2);
...
PROCEDURE GetBitIndex (e: Expr.T;  VAR i: INTEGER): BOOLEAN =
  ...
  BEGIN
    ...
    RETURN IntegerExpr.Split (e, x)
       AND TInt.ToInt (x, i)
       AND (0 <= i) AND (i <= Target.Longint.size);
  END GetBitIndex;
...
PROCEDURE GetBounds (ce: CallExpr.T;  VAR min, max: Target.Int) =
  ...
  BEGIN
    b := TInt.FromInt (Target.Longint.size, Target.Pre.Integer, int_size);
    <*ASSERT b*>
...
PROCEDURE Initialize () =
  VAR
    ...
    t  := ProcType.New (LInt.T, f0, f1, f2);
  BEGIN
    Z := CallExpr.NewMethodList (3, 3, TRUE, TRUE, TRUE, LInt.T,
                                 NIL,
