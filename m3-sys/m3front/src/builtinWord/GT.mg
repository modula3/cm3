...
PROCEDURE Compile (ce: CallExpr.T) =
  BEGIN
    ...
    CG.Compare (Word_types[rep].cg_type, CG.Cmp.GT);
  END Compile;
...
PROCEDURE PrepBR (ce: CallExpr.T;  true, false: CG.Label;  freq: CG.Frequency)=
  BEGIN
    ...
    Expr.Compile (ce.args[0]);
    Expr.Compile (ce.args[1]);
    CG.If_then (Word_types[rep].cg_type, CG.Cmp.GT, true, false, freq);
  END PrepBR;
...
PROCEDURE CompileMSIR (ce: CallExpr.T): MSIR.Value =
  ...
  BEGIN
    ...
    RETURN MSIR.BuildICmp (MSIRBuilder.CurrentBlock (), "", MSIR.CmpPred.Ugt, x, y);
  END CompileMSIR;
...
PROCEDURE Initialize (r: INTEGER) =
  ...
  BEGIN
    ...
    Procedure.DefinePredefined ("GT", Z, FALSE, t1, assignable:=TRUE);
