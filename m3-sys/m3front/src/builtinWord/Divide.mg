...
PROCEDURE CompileMSIR (ce: CallExpr.T): MSIR.Value =
  ...
  BEGIN
    ...
    RETURN MSIR.BuildIUDiv (MSIRBuilder.CurrentBlock (), "", x, y);
  END CompileMSIR;

PROCEDURE Fold (ce: CallExpr.T): Expr.T =
  VAR w0, w1, result: Target.Int;
  BEGIN
    IF GetArgs (ce.args, w0, w1) AND TWord.Div (w0, w1, result) THEN
...
PROCEDURE Initialize (r: INTEGER) =
  ...
  BEGIN
    ...
    Procedure.DefinePredefined ("Divide", Z, FALSE, t0, assignable:=TRUE);
