...
GENERIC MODULE FetchOr (Rep, Atomic);
  ...
  BEGIN
    ...
    ELSIF EnumExpr.Split (ce.args[2], order, t) AND TInt.ToInt (order, z) THEN
      ...
      | ORD(CG.MemoryOrder.Sequential) =>
        (* ok *)
      ELSE
        Error.Warn (0, "FetchOr currently only supports Order.Sequential");
      END;
    ELSE
...
PROCEDURE Compile (ce: CallExpr.T) =
  ...
  BEGIN
    ...
    CG.Fetch_and_op (CG.AtomicOp.Or,
...
PROCEDURE CompileMSIR (ce: CallExpr.T): MSIR.Value =
  ...
  BEGIN
    ...
    IF MSIR.BitWidth(elemT) < 0 THEN
      MSIRBuilder.Abandon("AtomicFetchOr on pointer type not supported in MSIR");
      RETURN NIL;
    ...
    END;
    old := MSIR.BuildAtomicRMW(b, "", MSIR.AtomicRMWOp.Or, ptr, val, ord);
    IF atomT # elemT THEN
