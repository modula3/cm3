...
IMPORT Text, Wx, Thread, Scheduler, Wr;
...
PROCEDURE PutMarkup(t: T;  text: Text.T;  width := 0) RAISES {Wr.Failure} =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, MarkUpOp);
    AddRef(t, text);
...
PROCEDURE DoPrintText(
  ...
  BEGIN
    ...
    IF mode = Mode.Writing THEN  t.wx.put(text)  END;
...
PROCEDURE DoPrintChar(
  ...
  BEGIN
    ...
    ELSE
      IF pos.b > 0 THEN DoTrailingBlanks(t, mode, pos) END;
      IF mode = Mode.Writing THEN t.wx.putChar(ch^) END;
      pos.c := pos.c + 1;
      RETURN (mode = Mode.Thinking) AND (pos.c > t.width);
    END;
  END DoPrintChar;
...
PROCEDURE DoTrailingBlanks(
  ...
  BEGIN
    IF mode = Mode.Writing THEN
      FOR i := 1 TO pos.b DO t.wx.putChar(' ') END;
    END;
    pos.c := pos.c + pos.b;
    pos.b := 0;
...
PROCEDURE DoNewLine(
  ...
  RAISES {Wr.Failure, Thread.Alerted} =
  BEGIN
    IF mode = Mode.Writing THEN t.wx.put (Wr.EOL) END;
    pos.c := 0;
