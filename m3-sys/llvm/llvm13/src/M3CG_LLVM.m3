...
REVEAL
  ...
  U = Public BRANDED "M3CG_LLVM.T" OBJECT
   ...
   (* external set functions *)
    ...
    debugRef      : LLVM.BuilderRef; (*M3DIB.DIBuilder;*)
    cuRef         : MetadataRef; (*M3DIB.DICompileUnit;*)
    fileRef       : MetadataRef; (*M3DIB.DIFile;*)
    funcRef       : MetadataRef; (*M3DIB.DISubprogram;*)
...
PROCEDURE IntrinsicFunc(m3Id : M3Intrinsic; numParms : CARDINAL; t1,t2,t3 : LLVM.TypeRef := NIL) : LLVM.ValueRef =
  ...
  BEGIN
    IF intrinId = 0 THEN
      ...
      IA[m3Id].id := intrinId;
    END;
    RETURN LLVM.LLVMGetIntrinsicDeclaration(modRef,intrinId,types,numParms);
  END IntrinsicFunc;
...
(* REVIEW: We always pass structs by reference at IR code level.
    ...
    (* add the dso_local attr  check this since this sets thread_local
       not dso
    LLVM.LLVMSetThreadLocal(mlProc.lvProc,TRUE);
    *)
    ...
    *)
    ...
    FOR i := 0 TO numParams - 1 DO
      ...
      (* Don't use byval attribute, because it is too hard to know when to
         ...
         NB: Not using byval breaks abi between llvm and gcc code. So we parameterize it. *)
      IF param.type = Type.Struct THEN
        LLVM.LLVMSetParamAlignment(lVal, param.align);
        IF llvmByval THEN
          LLVM.LLVMAddAttributeAtIndex(mlProc.lvProc, i+1, TypeAttr("byval",
            LLVM.LLVMGetElementType(LLVM.LLVMTypeOf(lVal))));
        END;
      END;
...
PROCEDURE declare_field (self: U; n: Name; o: BitOffset; s: BitSize; t: TypeUID; typeName : Name) =
  VAR
    recordRef : RecordDebug;
    debugObj : REFANY;
  ...
  BEGIN
    ...
    IF found AND ISTYPE(debugObj,PackedDebug) THEN
      packed := TRUE;
    END;
    recordRef.fields[recordRef.fieldIndex] := NEW(FieldDebug,name := n, bitOffset := VAL(o,LONGINT), tUid := t, bitSize := VAL(s,LONGINT), align := Align(s), packed := packed, eltTypeName := typeName);
...
PROCEDURE declare_global (self: U;  n: Name;  s: ByteSize;  a: Alignment; t: Type;  m3t: TypeUID;  exported, inited: BOOLEAN; <*UNUSED*>typeName : Name): Var =
  ...
  BEGIN
    ...
    LLVM.LLVMSetAlignment(v.lv, v.align);
    (* maybe set dso_local check this its not dso_local whatever that is
    LLVM.LLVMSetThreadLocal(v.lv,TRUE);
    *)
...
(* initializes the static variable at 'ADR(v)+o' with the address
  ...
  BEGIN
    ...
    labelObj := NEW(LabelVar, offset := o, value := value);
...
(* initializes the static variable at 'ADR(v)+o' with the integer
  ...
  BEGIN
    (* for the stack walker *)
    ofsObj := NEW(OfsVar, offset := o, value := value);
...
(* push; s0.A := ADR(v) + o *)
  ...
  BEGIN
    ...
    IF o # 0 THEN
      srcVal := BuildGep(srcVal,o,"load_adr");
    END;
...
(* s0.u := Mem [s0.A + o].t  *)
  ...
  BEGIN
    ...
    IF o # 0 THEN
      srcVal := BuildGep(srcVal,o,"load_ind");
    END;
...
(* Mem [s1.A + o].u := s0.t; pop (2) *)
  ...
  BEGIN
    ...
    IF o # 0 THEN
      destVal := BuildGep(destVal,o,"store_ind");
    END;
...
PROCEDURE CompareVal(a,b : LLVM.ValueRef; op : CompareOp; t : Type) : LLVM.ValueRef =
  ...
  BEGIN
    IF t = Type.Addr THEN
      a := LLVM.LLVMBuildBitCast(builderIR, a, AdrTy , LT("cmp_cast_a"));
      b := LLVM.LLVMBuildBitCast(builderIR, b, AdrTy , LT("cmp_cast_b"));
    END;
...
PROCEDURE IntAbs(a : LLVM.ValueRef; t : LLVM.TypeRef) : LLVM.ValueRef =
  ...
  BEGIN
    ...
    intMinPoison := LLVM.LLVMConstInt(LLVM.LLVMInt1Type(), VAL(0,LONGINT), TRUE);
    paramsRef := NewValueArr(paramsArr,numParams);
    paramsArr[0] := a;
    paramsArr[1] := intMinPoison;
    fn := IntrinsicFunc(M3Intrinsic.m3abs, numParams, t, t);
    res := LLVM.LLVMBuildCall(builderIR, fn, paramsRef, numParams, LT("abs"));
    RETURN res;
  END IntAbs;
...
PROCEDURE MinMax (self: U;  t: ZType; doMin : BOOLEAN) =
  ...
  BEGIN
    ...
    IF t < Type.Reel THEN
      ty := LLvmType(t);
      IF doMin THEN m3id := M3Intrinsic.m3smin; ELSE m3id := M3Intrinsic.m3smax; END;
    ELSE
      ty := FloatType(t);
      IF doMin THEN m3id := M3Intrinsic.m3fmin; ELSE m3id := M3Intrinsic.m3fmax; END;
    END;
    fn := IntrinsicFunc(m3id, numParams, ty, ty);
    res := LLVM.LLVMBuildCall(builderIR, fn, paramsRef, numParams, LT("minmax"));
    NARROW(s1,LvExpr).lVal := res;
    Pop(self.exprStack);
  END MinMax;
...
PROCEDURE DoRotate(value,shift : LLVM.ValueRef; t : LLVM.TypeRef; rotLeft : BOOLEAN) : LLVM.ValueRef =
  ...
  BEGIN
    ...
    res := LLVM.LLVMBuildCall(builderIR, fn, paramsRef, numParams, LT("rot"));
    RETURN res;
  END DoRotate;
...
PROCEDURE abort (self: U;  code: RuntimeError) =
  ...
  BEGIN
    ...
    paramsArr[0] := modVal;
    paramsArr[1] := codeVal;
    EVAL LLVM.LLVMBuildCall(builderIR, self.abortFunc.lvProc, paramsRef, numParams, LT(""));
  END abort;
...
(* SEE ALSO: load_static_link. *)
    ...
    ELSE
      ...
      lVal := LLVM.LLVMBuildInvoke
                (builderIR, fn, paramsRef, passedParamsCt,
...
PROCEDURE landing_pad (self : U; t: ZType; handler : Label;
  ...
  VAR
    landingVal,catchVal,clauseVal,excPtrVal,uidVal : LLVM.ValueRef;
  ...
  BEGIN
    ...
    (* this should always be i8* ie address *)
    ...
    FOR i := FIRST(catches) TO LAST(catches) DO
      ...
      END;
      clauseVal := LLVM.LLVMBuildBitCast (builderIR, catchVal, AdrTy, LT("clausecast"));
      LLVM.LLVMAddClause(landingVal, clauseVal);
    END;

    excPtrVal := LLVM.LLVMBuildExtractValue(builderIR, landingVal, 0, LT("exc_ptr"));
...
PROCEDURE InnerCallIndirect
  ...
  VAR
    ...
    numFormals : INTEGER;
    returnName : TEXT := "";
    byvals : REF ARRAY OF BOOLEAN := NIL;
  BEGIN
    ...
    IF numFormals > 0 THEN
      ...
      byvals := NEW(REF ARRAY OF BOOLEAN,numFormals);
      ...
      FOR paramNo := 0 TO numFormals - 1 DO
        ...
        IF llvmByval THEN
          byvals[paramNo] := FALSE;
          WITH ptype = LLVM.LLVMTypeOf(actual.lVal) DO
            IF LLVM.LLVMGetTypeKind(ptype) = LLVM.TypeKind.Pointer AND
               LLVM.LLVMGetTypeKind(LLVM.LLVMGetElementType(ptype)) =
                 LLVM.TypeKind.Struct THEN
              byvals[paramNo] := TRUE;
            END;
          END;
        END;
    ...
    ELSE
      ...
      catchLab.handler := TRUE;
      IF merge # NIL THEN
        resultVal := LLVM.LLVMBuildInvoke
                      (builderIR, callVal, paramsRef, numFormals,
    ...
    IF llvmByval AND byvals # NIL THEN
      FOR paramNo := 0 TO numFormals - 1 DO
        IF byvals[paramNo] THEN
          WITH lv = paramsArr[paramNo], ptype = LLVM.LLVMTypeOf(lv) DO
  ...
  BEGIN
    ...
    ELSE
      IF ptrBytes > TypeSize(t) THEN
        destTy := LLvmType(t);
        expr.lVal := LLVM.LLVMBuildTrunc(builderIR,expr.lVal,destTy, LT("pop_trunc"));
      END;
    END;
...
PROCEDURE DebugLine(self : U; line : INTEGER := 0) =
  ...
  BEGIN
    ...
    IF self.funcRef # NIL THEN
      ...
      mloc := M3DIB.CreateDebugLocation(globContext,self.curLine,0,scope,NIL);
...
(* FIXME ^ for compiler generated procedure inits *)
    ...
    IF proc.returnType = Type.Void THEN
      tyVal := NIL; (*LOOPHOLE(M3DIB.DITypeEmpty,MetadataRef);*)
...
: MetadataRef =
  VAR
    LDIType, LDISubprogTy, PTy : MetadataRef;
    paramsArr : REF ARRAY OF MetadataRef;
    paramsMetadata : LLVMTypes.ArrayRefOfMetadataRef;
    typeName : TEXT;
  BEGIN
...
(*
FIXME^ - need to handle recursive proc type
     Need a replaceable subroutine type in llvm. Actually pointer type
     TYPE T = PROCEDURE (t: T); fails with stack overflow
*)
...
(* does not work
    tempTy := DebugLookupLL(self,UID_INTEGER);
...
(*
    ...
    M3DIB.LLVMMetadataReplaceAllUsesWith(
      TempTargetMetadata := tempTy,
      Replacement        := LDISubprogTy);
*)
    p.diType := PTy;
    RETURN PTy;
  END DebugProc;
  ...
  BEGIN
    ...
    END;
     ...
     expr := M3DIB.CreateExpression(self.debugRef,
                paramsInt64.Data, paramsInt64.Length);
    RETURN expr;
  END Expr;
