...
// Bindings for LLVM DIBuilder version 3.9, 4.0.1 and 5.0.0

%pragma(modula3) unsafe="true";

/* 5.0 introduced conversion macros in attempt at C binding
but swig was not impressed so redifine it to nothing */
...
//Optional is an enum class - just ignore it
%typemap("m3rawintype")   Optional<unsigned>  %{ADDRESS%}
%typemap("m3wrapintype")  Optional<unsigned>  %{ADDRESS%}
...
%rename(insertValueAtEnd) llvm::DIBuilder::insertDbgValueIntrinsic(llvm::Value *, uint64_t, DILocalVariable *, DIExpression *, const DILocation *, BasicBlock *);
%rename(insertValueBefore) llvm::DIBuilder::insertDbgValueIntrinsic(llvm::Value *, uint64_t, DILocalVariable *, DIExpression *, const DILocation *, Instruction *);
...
%ignore createCompileUnit;
%ignore createFile;
%ignore createEnumerator;
%ignore createUnspecifiedType;
%ignore createNullPtrType;
%ignore createBasicType;
%ignore createQualifiedType;
%ignore createPointerType;
%ignore createMemberPointerType;
%ignore createReferenceType;
%ignore createTypedef;
%ignore createFriend;
%ignore createInheritance;
%ignore createMemberType;
%ignore createBitFieldMemberType;
%ignore createStaticMemberType;
%ignore createObjCIVar;
%ignore createObjCProperty;
%ignore createClassType;
%ignore createStructType;
%ignore createUnionType;
%ignore createTemplateTypeParameter;
%ignore createTemplateValueParameter;
%ignore createTemplateTemplateParameter;
%ignore createTemplateParameterPack;
%ignore createArrayType;
%ignore createVectorType;
%ignore createEnumerationType;
%ignore createSubroutineType;
%ignore createExternalTypeRef;
%ignore createArtificialType;
%ignore createObjectPointerType;
%ignore createForwardDecl;
%ignore createReplaceableCompositeType;
%ignore retainType;
%ignore createUnspecifiedParameter;
%ignore getOrCreateArray;
%ignore getOrCreateTypeArray;
%ignore getOrCreateSubrange;
%ignore createGlobalVariable;
%ignore createTempGlobalVariableFwdDecl;
%ignore createAutoVariable;
%ignore createParameterVariable;
%ignore createExpression;
%ignore createBitPieceExpression;
%ignore createFunction;
%ignore createTempFunctionFwdDecl;
%ignore createMethod;
%ignore createNameSpace;
%ignore createModule;
%ignore createLexicalBlockFile;
%ignore createLexicalBlock;
%ignore createImportedModule;
%ignore createImportedDeclaration;
%ignore insertDeclare;
%ignore insertDbgValueIntrinsic;
%ignore replaceVTableHolder;
%ignore replaceArrays;
%ignore finalize;

//4.0.1

%ignore createMacro;
%ignore createTempMacroFile;
%ignore getOrCreateMacroArray;
%ignore createGlobalVariableExpression;
%ignore createFragmentExpression;
%ignore createConstantValueExpression;

//5.0.0

%ignore finalizeSubprogram;
