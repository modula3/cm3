...
TYPE
  ...
  BaseDebug = OBJECT
    ...
    typeName : Name;
    encoding : unsigned;
    DIDescr : M3DIB.DIType := NIL; 
(* ^ CHECK - Descriptor is gone in llvm 5.0, so am guessing its now Type
could be wrong though. And maybe change name from DIDescr to DIType ? *)
  END;
  ...
  ObjectDebug = RecordDebug OBJECT
    ...
    objectDescr : M3DIB.DIType := NIL; (* object DIType - not pointer *)
    brand : TEXT;
    traced : BOOLEAN;
...
(* amd64 *)
  ...
  dataRep := "e-m:e-p:64:64-i64:64-f80:128-n8:16:32:64-S128";

(* linuxlibc6 
  targetTriple := "i686-pc-linux-gnu";
  dataRep := "e-m:e-p:32:32-i32:32-i64:32:64-f80:128-n8:16:32:64-S128";
*)
...
PROCEDURE EnumAttr(name : TEXT) : LLVM.AttributeRef =
  VAR
    attrKind : CARDINAL;
  BEGIN
    (* names are like nest, byval or sext*)
    attrKind := LLVM.LLVMGetEnumAttributeKindForName(LT(name), Text.Length(name));
    RETURN  LLVM.LLVMCreateEnumAttribute(globContext, attrKind, 0L);
  END EnumAttr;
...
PROCEDURE DoRotate(value,shift : LLVM.ValueRef; rotLeft : BOOLEAN) : LLVM.ValueRef =
  ...
  BEGIN
    ...
    intTy := LLVM.LLVMTypeOf(value);
    typeWidth := VAL(LLVM.LLVMGetIntTypeWidth(intTy),LONGINT);
    maskSize := LLVM.LLVMConstInt(intTy, typeWidth - 1L, TRUE);
...
(* CHECK ^ Could create a new Var and call AllocVarInEntryBB
at very least have to turn off debugging around the alloc *)
    ...
    IF self.genDebug THEN
      IF self.funcRef # NIL THEN
        ...
        ELSE
          scope := self.funcRef;
        END;
        mloc := LLVM.DIBGetDebugLoc(self.curLine,0,self.funcRef);
...
PROCEDURE DebugPushBlock(self : U) =
  VAR
    DILexBlock : M3DIB.DILexicalBlock; 
    parent : M3DIB.DISubprogram;
    blockRef : BlockDebug;
  BEGIN
    IF NOT self.genDebug THEN RETURN; END;
    IF self.debugLexStack.size() = 0 THEN
    ...
    ELSE
      ...
      DILexBlock := self.debugRef.createLexicalBlock
         (parent, self.fileRef, self.curLine, 0);         
...
: M3DIB.DISubrange =
  ...
  BEGIN
    ...
    Result 
      := self.debugRef.getOrCreateSubrange
...
PROCEDURE DebugOpenArray
  ...
  BEGIN
    ...
    *)
    ...
    subsVal 
      := self.debugRef.getOrCreateSubrange(0L,100L);
      (*
           (TIntToint64_t(0), TIntToint64_t(100)); 
      *)
    NewArrayRefOfMetadataRef(1, (*OUT*)paramsArr, (*OUT*)paramsMetadata);
    paramsArr[0] := subsVal;
    paramsDIArr := self.debugRef.getOrCreateArray(paramsMetadata);
    ...
    oarrDIT :=
      self.debugRef.createArrayType(VAL(a.bitSize,uint64_t), VAL(a.align,uint32_t), eltVal, paramsDIArr);
...
PROCEDURE DebugEnum(self : U; e : EnumDebug) : M3DIB.DICompositeType =
  ...
  BEGIN
    ...
    IF e.typeName = M3ID.NoID THEN
      eName := "Enum";
(* FIXME ^ If the enum is imported from another module
there is no declare_typename *)      
    ELSE
      eName := M3ID.ToText(e.typeName);
    END;
...
PROCEDURE DebugLookupML(self : U; tUid : TypeUID) : BaseDebug =
  ...
  BEGIN
    ...
    ELSE (* Complete this ML node's dependencies, then itself. *) 
      ...
      | SubrangeDebug(d) => LDIDescr := 
        DebugBasic(self,d);
      (*DebugSubrange(self,d);*
CHECK ^ llc gives invalid type ref. Maybe subrange is
just for arrays. Or else limitation of llvm debug info*)
...
<*UNUSED*>PROCEDURE SetExprValue(self : U; var : M3DIB.DILocalVariable; value : LLVM.ValueRef) =
  ...
  BEGIN
(*
  test setting a value - corresponds to llvm.dbg.value
*)
    loc := LLVM.DIBGetDebugLoc(self.curLine,0,self.funcRef);
    diLoc := LOOPHOLE(loc,M3DIB.DILocation);
    
    NewArrayRefOfint64
      (2, (*OUT*)paramsArr, (*OUT*)paramsInt64);
    (*
     simple test of address offset expression
    *)
    paramsArr[0] := VAL(DC.DW_OP_plus_uconst,LONGINT);      
    paramsArr[1] := 16L;
    exp := self.debugRef.createExpression2(paramsInt64);
 
    EVAL self.debugRef.insertValueAtEnd (
              Val        := value,
              Offset     := ptrBits, (* whats this *)
              VarInfo    := var, (*the variable *)
              Expr       := exp,
              DL         := diLoc,
...
PROCEDURE GetExpr(self : U; v : LvVar) : M3DIB.DIExpression =
  ...
  BEGIN
    ...
    IF ISTYPE(debugObj,IndirectDebug) OR
      ...
      paramsArr[0] := VAL(DC.DW_OP_deref,LONGINT);      
      exp := self.debugRef.createExpression2(paramsInt64);
(* delete or fix this for other types
    ELSIF ISTYPE(debugObj,OpenArrayDebug) THEN
    (* this case never activated since all open arrays
    are indirect. Maybe useful for other cases though*)
      NewArrayRefOfint64
      (2, (*OUT*)paramsArr, (*OUT*)paramsInt64);

      (* move address of the var past the header *)
      paramsArr[0] := VAL(DC.DW_OP_plus_uconst,LONGINT);      
      paramsArr[1] := 16L;
      exp := self.debugRef.createExpression2(paramsInt64);      
*)
...
(* Review: Actually probably so for static link, and maybe result. *) 
    ...
    SetExprValue(self,lvDebug,v.lv);
    *)
...
(* Fix this. Its an attempt to get debug info for
all types, especially for interfaces that in the absence
of parameters or globals that reference them would leave
an object file without debugging info. 
PROCEDURE DebugAllTypes(self : U) =
  VAR
    iter : IntRefTbl.Iterator;
    key : INTEGER;
    debObj : REFANY;
  ...
  BEGIN
    ...
    WHILE iter.next(key, debObj) DO
      debug := NARROW(debObj,BaseDebug);
      EVAL DebugLookupLL(self, debug.tUid);
