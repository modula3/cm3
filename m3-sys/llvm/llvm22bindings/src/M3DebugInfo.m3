...
PROCEDURE FinalizeSubprogram
  ...
  BEGIN
    M3DebugInfoRaw.FinalizeSubprogram(Builder, Subprogram);
  END FinalizeSubprogram;
...
PROCEDURE CreateImportedModuleFromAlias
  ...
  BEGIN
    RETURN
      M3DebugInfoRaw.CreateImportedModuleFromAlias(
        Builder, Scope, ImportedEntity, File, Line, Elements, NumElements);
...
PROCEDURE CreateImportedModuleFromModule
  ...
  BEGIN
    RETURN M3DebugInfoRaw.CreateImportedModuleFromModule(
             Builder, Scope, M, File, Line, Elements, NumElements);
...
PROCEDURE CreateImportedDeclaration (Builder          : BuilderRef;
  ...
  BEGIN
    ...
    result := M3DebugInfoRaw.CreateImportedDeclaration(
                Builder, Scope, Decl, File, Line, arg6, NameLen, Elements,
                NumElements);
...
PROCEDURE CreateSubrangeType
  ...
  BEGIN
    arg3 := M3toC.SharedTtoS(Name);
    arg9tmp := ORD(Flags);
    result := M3DebugInfoRaw.CreateSubrangeType(
                Builder, Scope, arg3, NameLen, LineNo, File, SizeInBits,
                AlignInBits, arg9tmp, BaseTy, LowerBound, UpperBound,
                Stride, Bias);
    M3toC.FreeSharedS(Name, arg3);
    RETURN result;
  END CreateSubrangeType;
...
PROCEDURE CreateDynamicArrayType
  ...
  BEGIN
    arg3 := M3toC.SharedTtoS(Name);
    result := M3DebugInfoRaw.CreateDynamicArrayType(
                Builder, Scope, arg3, NameLen, LineNo, File, Size,
                AlignInBits, Ty, Subscripts, NumSubscripts, DataLocation,
                Associated, Allocated, Rank, BitStride);
    M3toC.FreeSharedS(Name, arg3);
    RETURN result;
...
PROCEDURE CreateObjectPointerType
  ...
  MetadataRef =
  BEGIN
    RETURN M3DebugInfoRaw.CreateObjectPointerType(Builder, Type, Implicit);
...
PROCEDURE LLVMGetDINodeTag (MD: MetadataRef; ): INTEGER =
  BEGIN
    RETURN M3DebugInfoRaw.LLVMGetDINodeTag(MD);
  END LLVMGetDINodeTag;
...
PROCEDURE LLVMDISubprogramReplaceType
  ...
  BEGIN
    M3DebugInfoRaw.LLVMDISubprogramReplaceType(Subprogram, SubroutineType);
  END LLVMDISubprogramReplaceType;
...
PROCEDURE CreateLabel (Builder       : BuilderRef;
  ...
  BEGIN
    arg3 := M3toC.SharedTtoS(Name);
    result := M3DebugInfoRaw.CreateLabel(Builder, Context, arg3, NameLen,
                                         File, LineNo, AlwaysPreserve);
    M3toC.FreeSharedS(Name, arg3);
    RETURN result;
  END CreateLabel;
...
PROCEDURE InsertLabelBefore (Builder            : BuilderRef;
  ...
  DbgRecordRef =
  BEGIN
    RETURN M3DebugInfoRaw.InsertLabelBefore(
             Builder, LabelInfo, Location, InsertBefore);
  END InsertLabelBefore;
...
PROCEDURE InsertLabelAtEnd (Builder            : BuilderRef;
  ...
  DbgRecordRef =
  BEGIN
    RETURN M3DebugInfoRaw.InsertLabelAtEnd(
             Builder, LabelInfo, Location, InsertAtEnd);
  END InsertLabelAtEnd;
