...
func(xb.l) is opposite sign from func(xb.r).  Grow outward from
the original xb.l,xb.r by golden ratio, for geometric growth.
Return true if a good xb.l,xb.r can be found before getting to
...
PROCEDURE BracketIn(func:Ftn;       (*find brackets for this function*)
          ...
          VAR (*OUT*) xs:ARRAY OF Bracket;  (*returning pairs here*)
                    ):CARDINAL      (*with this count of valid pairs,
                                      the rest of the array won't be touched*)
                    RAISES {Error}=
<*UNUSED*> CONST ftn = Module & "BracketIn";
VAR
  h,x,xh,y,yh:R.T;
  ns:CARDINAL:=0;
BEGIN
...
(*Given brackets xb.l,xb.r, find a root via bisection, and refine it
to within +/- tol
*)
<*UNUSED*> CONST
...
bracket points.  xb.l and xb.r must be of opposite signs.
*)
<*UNUSED*> CONST
  ftn = Module & "Brent";
VAR
...
BEGIN
  ...
  FOR i:=1 TO maxiter DO
    ...
    diffnext:=RT.Half*(c-b);  diff2:=R.Two*diffnext;
...
PROCEDURE NewtonRaphson(
                 ...
                 READONLY xb:Bracket;    (*bracketed by these points*)
...
and brackets xb.l and xb.r, find the root to xtol precision.
Works via newton-raphson and bisection.

*)
<*UNUSED*> CONST ftn = Module & "NewtonRaphson";
VAR
  y:DerivativeArray2;
...
BEGIN
  ...
  VAR
    y1,y2:R.T;
  BEGIN
    y:=func(xb.l); IF y[0]=R.Zero THEN RETURN xb.l; END; y1:=y[0];
    y:=func(xb.r); IF y[0]=R.Zero THEN RETURN xb.r; END; y2:=y[0];
    IF (y1<R.Zero AND y2<R.Zero) OR (y1>R.Zero AND y2>R.Zero) THEN
      RAISE Error(Err.not_bracketed);
    END;
