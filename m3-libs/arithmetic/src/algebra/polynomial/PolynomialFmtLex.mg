...
PROCEDURE TexPower (i : CARDINAL; x : R.T; READONLY style : TexStyle) : TEXT =
  BEGIN
    RETURN RF.Tex(x,style.elemStyle,Precedence.product) & " " &
           style.var & "^{" & F.Int(i) & "}";
  END TexPower;

PROCEDURE TexSimplePower (i : CARDINAL; x : R.T; READONLY style : TexStyle) : TEXT =
  BEGIN
    IF i=0 THEN
      RETURN RF.Tex(x,style.elemStyle,Precedence.sum);
    ELSIF i=1 THEN
      RETURN RF.Tex(x,style.elemStyle,Precedence.product) & " " &
             style.var;
    ELSE
      RETURN TexPower(i,x,style);
    END;
  END TexSimplePower;

PROCEDURE TexCoefficient (<*UNUSED*> i : CARDINAL; x : R.T; READONLY style : TexStyle) : TEXT =
  BEGIN
    RETURN RF.Tex(x,style.elemStyle,Precedence.sum);
  END TexCoefficient;
...
VAR
  wr:=TextWr.New();
  sep:TEXT;
  texCoef:PROCEDURE (i : CARDINAL; x : R.T; READONLY style : TexStyle) : TEXT;
BEGIN
  ...
  ELSE
    IF TexFlag.reverse IN style.flags THEN
      ...
      BEGIN
        LOOP
          DEC(i);
          Wr.PutText(wr,texCoef(i,x[i],style));
