...
(*Copyright (c) 1996, m3na project Abstract: Tests for Integer module.
...
PROCEDURE TestSqRt (): BOOLEAN =
  ...
  BEGIN
    ...
    FOR i := 0 TO 10 DO
      ii := i * i;
      iisqrt := IT.SqRt(ii);
      Msg("SqRt(" & CdF.Fmt(ii) & ")=" & CdF.Fmt(i) & "=" & CdF.Fmt(iisqrt)
            & "\n");
    END;
...
PROCEDURE TestCordic (): BOOLEAN =
  ...
  BEGIN
    ...
    FOR i := 1 TO n DO
      ...
      truesin := RT.Sin(rad);
      truecos := RT.Cos(rad);
      Msg("truecos=" & RF.Fmt(truecos) & " truesin=" & RF.Fmt(truesin)
            & "\n");
      ...
      Msg("calccos=" & RF.Fmt(calccos) & " calcsin=" & RF.Fmt(calcsin)
            & "\n\n");
    END;
    RETURN result;
  END TestCordic;
