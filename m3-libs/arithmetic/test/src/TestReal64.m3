...
PROCEDURE TestLnGamma (): BOOLEAN =
  ...
  BEGIN
    ...
    IF NOT Verify(ftn, "LnGamma", SF.LnGamma(10.0d0), 12.80182748008147D0,
                  1.0d4) THEN
      result := FALSE;
    END;
    RETURN result;
...
PROCEDURE TestBetaI (): BOOLEAN =
  ...
  BEGIN
    ...
    FOR i := 1 TO 9 DO
      ...
      Msg("BetaI(1.0d0,3.0d0," & RF.Fmt(ix) & ")=" & RF.Fmt(b) & "\n");
    END;

    IF NOT Verify(
             ftn, "BetaI", SF.BetaI(1.0d0, 3.0d0, 0.2d0), 0.488d0, 0.01d0) THEN
      result := FALSE;
    END;
    RETURN result;
  END TestBetaI;
...
PROCEDURE TestGammaP (): BOOLEAN =
  ...
  BEGIN
    ...
    FOR i := 1 TO 9 DO
      ...
      y := SF.GammaP(a, ix);
      Msg(
        "GammaP(" & RF.Fmt(a) & "," & RF.Fmt(ix) & ")=" & RF.Fmt(y) & "\n");
    END;
...
PROCEDURE TestGammaQ (): BOOLEAN =
  ...
  BEGIN
    ...
    FOR i := 1 TO 9 DO
      ...
      Msg("1-GammaQ(" & RF.Fmt(a) & "," & RF.Fmt(ix) & ")=" & RF.Fmt(y)
            & "\n");
    END;

    IF NOT Verify(ftn, "GammaQ", SF.GammaQ(3.0d0, 2.0d0), R.One - 0.323d0,
                  0.01d0) THEN
      result := FALSE;
    END;
    RETURN result;
