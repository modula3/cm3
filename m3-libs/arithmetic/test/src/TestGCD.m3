...
PROCEDURE TestComplexGCD (): BOOLEAN =
  ...
  BEGIN
    Debug(1, ftn, "begin\n");
    (* this is an example where the naive GCD algorithm run into an
       infinite loop x := IC.T{4,1}; y := IC.T{4,-1}; Msg(F.FN("gcd(%s,%s)
       ?\n", ARRAY OF TEXT{ICF.Fmt(x), ICF.Fmt(y)})); VAR q,r : ICG.T;
       BEGIN q := IC.DivMod(x,y,r); Msg(F.FN("q=%s,r=%s\t", ARRAY OF
       TEXT{ICF.Fmt(q), ICF.Fmt(r)})); q := IC.DivMod(y,x,r);
       Msg(F.FN("q=%s,r=%s\n", ARRAY OF TEXT{ICF.Fmt(q), ICF.Fmt(r)}));
       END; EVAL ICG.GCD(x,y); *)
...
PROCEDURE TestPolynomialGCD (): BOOLEAN =
  CONST ftn = Module & "TestPolynomialGCD";
  VAR
    fac0, fac1, poly0, poly1, gcd: BFrP.T;
    result                                := TRUE;
  BEGIN
    Debug(1, ftn, "begin\n");
    ...
    fac1 := BFrP.New(3);
    fac1^ :=
      BFrP.TBody{IntToFrac(1), IntToFrac(2), IntToFrac(3), IntToFrac(1)};
