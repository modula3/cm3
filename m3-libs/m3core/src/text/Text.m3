...
PROCEDURE Equal (t, u: T): BOOLEAN =
  VAR Result : BOOLEAN; 
  BEGIN
    TextClass.NoteGround (TextClass.Op.Equal); 
    IF TextClass.Old 
    THEN Result := OldEqual (t, u);
    ELSE Result := NewEqual (t, u);
    END;
    TextClass.NoteFinished (TextClass.Op.Equal); 
    RETURN Result
  END Equal;
...
PROCEDURE EqualBuf (t, u: TEXT;  len: CARDINAL): BOOLEAN =
  ...
  BEGIN
    ...
    WHILE (i <= j) DO
      ...
      INC (i, NUMBER (buf_t));
      TextClass.NoteIter (TextClass.Op.Equal); 
    END;
...
PROCEDURE Compare (t, u: T): [-1..1] =
  VAR Result : [-1..1]; 
  BEGIN
    TextClass.NoteGround (TextClass.Op.Compare); 
    IF TextClass.Old 
    THEN Result := OldCompare (t, u);
    ELSE Result := NewCompare (t, u);
    END;
    TextClass.NoteFinished (TextClass.Op.Compare); 
    RETURN Result
  END Compare;
...
PROCEDURE CompareBuf (t, u: TEXT;  len_t, len_u: CARDINAL): [-1 .. 1] =
  ...
  BEGIN
    WHILE (i < min_len) DO
      ...
      INC (i, NUMBER (buf_t));
      TextClass.NoteIter (TextClass.Op.Compare); 
    END;
...
PROCEDURE Hash (t: T): Word.T =
  VAR Result : Word.T; 
  BEGIN
    TextClass.NoteGround (TextClass.Op.Hash); 
    IF TextClass.Old 
    THEN Result := OldHash (t);
    ELSE Result := NewHash (t);
    END;
    TextClass.NoteFinished (TextClass.Op.Hash); 
    RETURN Result
  END Hash;
...
PROCEDURE HashBuf (t: T;  len: CARDINAL): Word.T =
  ...
  BEGIN
    WHILE start < len DO
      ...
      TextClass.NoteIter (TextClass.Op.Hash); 
    END;
    RETURN result;
  END HashBuf;
...
PROCEDURE HasWideCharsBuf16 (t: T;  len: CARDINAL): BOOLEAN =
  ...
  BEGIN
    WHILE start < len DO
      ...
      ELSE 
        INC (start, NUMBER (buf));
        TextClass.NoteIter (TextClass.Op.HasWideChars); 
      END; 
    END;
    RETURN FALSE;
...
PROCEDURE GetWideChar (t: T; i: CARDINAL): WIDECHAR =
  VAR Result : WIDECHAR;
  BEGIN
    TextClass.NoteGround (TextClass.Op.GetWideChar); 
    TextClass.NoteGround (TextClass.Op.get_wide_char); 
    Result := t.get_wide_char (i);
    TextClass.NoteFinished (TextClass.Op.get_wide_char); 
    TextClass.NoteFinished (TextClass.Op.GetWideChar); 
    RETURN Result 
  END GetWideChar;

PROCEDURE SetChars (VAR a: ARRAY OF CHAR;  t: T;  start: CARDINAL) =
  BEGIN
    TextClass.NoteGround (TextClass.Op.SetChars); 
    TextClass.NoteGround (TextClass.Op.get_chars); 
    t.get_chars (a, start);
    TextClass.NoteFinished (TextClass.Op.get_chars); 
    TextClass.NoteFinished (TextClass.Op.SetChars); 
  END SetChars;

PROCEDURE SetWideChars (VAR a: ARRAY OF WIDECHAR;  t: T;  start: CARDINAL) =
  BEGIN
    TextClass.NoteGround (TextClass.Op.SetWideChars); 
    TextClass.NoteGround (TextClass.Op.get_wide_chars); 
    t.get_wide_chars (a, start);
    TextClass.NoteFinished (TextClass.Op.get_wide_chars); 
    TextClass.NoteFinished (TextClass.Op.SetWideChars); 
  END SetWideChars;
...
PROCEDURE FromChar (c: CHAR): T =
  VAR Result: T; 
  BEGIN
    TextClass.NoteGround (TextClass.Op.FromChar); 
    Result := FromCharInner (c);
    TextClass.NoteFinished (TextClass.Op.FromChar); 
    RETURN Result;
  END FromChar;
...
PROCEDURE FromChars (READONLY a: ARRAY OF CHAR): T =
  ...
  VAR Result: T; 
  BEGIN
    TextClass.NoteGround (TextClass.Op.FromChars); 
    IF n = 0 THEN Result := "" 
    ELSIF n = 1 THEN Result := FromCharInner (a [0])
    ELSE Result := Text8.New (a);
    END; 
    TextClass.NoteFinished (TextClass.Op.FromChars); 
    RETURN Result;
  END FromChars;
...
PROCEDURE FromWideChars (READONLY a: ARRAY OF WIDECHAR): T =
  ...
  VAR Result: T; 
  BEGIN
    TextClass.NoteGround (TextClass.Op.FromWideChars); 
    IF n = 0 THEN Result := ""
    ELSIF n = 1 THEN Result := FromWideChar (a [0])
    ELSE Result := Text16.New (a)
    END; 
    TextClass.NoteFinished (TextClass.Op.FromWideChars); 
    RETURN Result;
  END FromWideChars;

VAR FindCharOp: TextClass.Op; 

PROCEDURE FindChar (t: T;  c: CHAR;  start := 0): INTEGER =
  VAR Result : INTEGER; 
  BEGIN
    FindCharOp := TextClass.Op.FindChar;
    TextClass.NoteGround (TextClass.Op.FindChar); 
    IF TextClass.Old
    THEN Result := OldFindChar (t, c, start); 
    ELSE Result := NewFindChar (t, c, start);
    END;
    TextClass.NoteFinished (TextClass.Op.FindChar); 
    RETURN Result
  END FindChar; 
...
PROCEDURE FindWideChar (t: T;  c: WIDECHAR;  start := 0): INTEGER =
  VAR Result : INTEGER; 
  BEGIN
    FindCharOp := TextClass.Op.FindWideChar;
    TextClass.NoteGround (TextClass.Op.FindWideChar); 
    IF TextClass.Old
    THEN Result := OldFindWideChar (t, c, start); 
    ELSE Result := NewFindWideChar (t, c, start);
    END;
    TextClass.NoteFinished (TextClass.Op.FindWideChar); 
    RETURN Result
  END FindWideChar; 
...
PROCEDURE FindCharR (t: T;  c: CHAR; start := LAST (INTEGER)): INTEGER =
  VAR Result : INTEGER; 
  BEGIN
    FindCharOp := TextClass.Op.FindCharR;
    TextClass.NoteGround (TextClass.Op.FindCharR); 
    IF TextClass.Old
    THEN Result := OldFindCharR (t, c, start); 
    ELSE Result := NewFindCharR (t, c, start);
    END;
    TextClass.NoteFinished (TextClass.Op.FindCharR); 
    RETURN Result
  END FindCharR; 
...
PROCEDURE FindWideCharR 
  ...
  VAR Result : INTEGER; 
  BEGIN
    FindCharOp := TextClass.Op.FindWideCharR;
    TextClass.NoteGround (TextClass.Op.FindWideCharR); 
    IF TextClass.Old
    THEN Result := OldFindWideCharR (t, c, start); 
    ELSE Result := NewFindWideCharR (t, c, start);
    END;
    TextClass.NoteFinished (TextClass.Op.FindWideCharR); 
    RETURN Result
  END FindWideCharR; 
