...
PROCEDURE Concat (t, u: TEXT): TEXT =
  ...
  VAR Result: TEXT := NIL;
  BEGIN
    TextClass.NoteGround (TextClass.Op.Cat); 
    IF TextClass.Old 
    ...
    ELSE (* The nonrecursive part of the new Concat: *)  
      ...
      END (* IF *) 
    END (* IF *);
    TextClass.NoteFinished (TextClass.Op.Cat); 
    RETURN Result 
  END Concat;
...
PROCEDURE OldConcat (t, u: TEXT): TEXT =
  ...
  BEGIN
    ...
    TextClass.NoteAllocTextCat(Result); 
    RETURN Result 
  END OldConcat;
...
PROCEDURE Balance 
     ( LText: TEXT; VAR LInfo : TextClass.Info; 
       ...
       (* ^If non-NIL, a node that is TextCat.T(LRext,RText), which Balance
          can return if it decides this is its best balanced result.
        *) 
  ...
  PROCEDURE Cat_L_R ( ) = (* Never iterates, caller should always RETURN. *) 
    BEGIN 
      ...
      THEN (* Build a new Cat node. *) 
        ...
        ResultRef ^ := Cat;
        TextClass.NoteAllocTextCat (Cat); 
      ELSE
        ResultRef ^ := CatNode; 
      END; 
    END Cat_L_R; 
    ...
    BEGIN 
      ...
      THEN 
        ...
        (* Tail recursion elimination of: 
          ...
          RETURN 
        *)
        TextClass.NoteIter (TextClass.Op.Cat); 
      ...
      ELSE (* Avoid risk of creating a copy of CatNode, at the cost of 
           ...
           *) 
        TextClass.NoteRecurse (TextClass.Op.Cat); 
        Balance 
            ...
            THEN (* Were able to combine and flatten LRText and RLText 
              ...
              ELSE (* Cat(LL,Bal(Flat(LR,RL),RR)) *) 
                ...
                *)
                ...
                (* And loop. *) 
              END (* IF *)
            ELSE (* Couldn't combine LRText with RLText. *)  
              ...
              THEN (* LRText is flat. *) 
                ...
                THEN (* Both LRText and RLText are already flat, but could
                  ...
                  ResultRef ^ := Cat;
                  TextClass.NoteAllocTextCat (Cat); 
                  TextClass.SaveRecurseDepth (TextClass.Op.Cat, RDepth);
                  TextClass.NoteRecurse (TextClass.Op.Cat); 
                  ...
                  THEN (* Recurse on as-short left side, iterate on right. *) 
                    Balance 
                      (LLText, LLInfo, LRText, LRInfo, ADR (Cat.a), 
                       CatNode := LCat);
                    TextClass.RestoreRecurseDepth (TextClass.Op.Cat, RDepth);
                    (* Tail recursion elimination of: 
                      Balance 
...
(* Maybe flatten left and right into a single heap object. 
   If did not flatten, return NIL, with possible changes
   made to ResultInfo, but they are undefined and irrelevant.
*) 
  ...
  BEGIN
    ...
    THEN 
      ...
      THEN (* Flatten into a Text16Short.T. *) 
        ...
        TextClass.NoteAllocText16Short(ResultText16Short); 
    ...
    ELSE (* No wide chars involved. *) 
      ...
      THEN (* Flatten into a Text8Short.T. *) 
        ...
        TextClass.NoteAllocText8Short(ResultText8Short); 
  ...
  BEGIN
    LOOP 
      ...
      THEN (* Dispatch and Tail-recursion elimination of the code in the
              ELSE clause below. *) 
        TextClass.NoteIter (TextClass.Op.get_char); 
        Text := Child;
...
PROCEDURE CatNewGetWideChar (Text: T;  index: CARDINAL): WIDECHAR =
  ...
  BEGIN
    LOOP 
      ...
      THEN (* Dispatch and Tail-recursion elimination of the code in the
              ELSE clause below. *) 
        TextClass.NoteIter (TextClass.Op.get_wide_char); 
        Text := Child;
        (* index is already set. *) 
        (* And loop. *)  
      ELSE 
...
PROCEDURE OldGetChars (t: T;  VAR a: ARRAY OF CHAR;  start: CARDINAL) =
  ...
  BEGIN
    ...
    IF (t_offset + t.a_len > start) THEN
      ...
      TextClass.SaveRecurseDepth (TextClass.Op.get_chars, RDepth); 
      TextClass.NoteRecurse (TextClass.Op.get_chars); 
      u.get_chars (SUBARRAY (a, a_offset, NUMBER (a) - a_offset), u_offset);
      TextClass.RestoreRecurseDepth (TextClass.Op.get_chars, RDepth); 
...
PROCEDURE MiddleGetChars 
  (Text: T;  VAR Chars: ARRAY OF CHAR;  start: CARDINAL) =
  VAR Chars_offset, b_offset: CARDINAL;
  VAR RDepth: CARDINAL; 
  BEGIN 
    IF start < Text.a_len  THEN
      TextClass.SaveRecurseDepth (TextClass.Op.get_chars, RDepth); 
      TextClass.NoteRecurse (TextClass.Op.get_chars); 
      Text.a.get_chars (SUBARRAY (Chars, 0, NUMBER (Chars)), start);
      TextClass.RestoreRecurseDepth (TextClass.Op.get_chars, RDepth); 
      Chars_offset := Text.a_len - start;
      IF Chars_offset >= NUMBER (Chars) THEN RETURN; END;
    ELSE Chars_offset := 0
    END;

    IF start < Text.a_len + Text.b_len THEN
      b_offset := MAX (start - Text.a_len, 0);
      TextClass.NoteRecurse (TextClass.Op.get_chars); 
      Text.b.get_chars 
...
PROCEDURE NewGetChars (Text: T;  VAR Chars: ARRAY OF CHAR;  start: CARDINAL) =
        ...
        THEN
          ...
          THEN 
            (* Dispatch and Tail-recursion elimination of the code in the
               ELSE clause below. *) 
            TextClass.NoteIter (TextClass.Op.get_chars); 
            Text := Text.a;
...
PROCEDURE OldGetWideChars (t: T;  VAR a: ARRAY OF WIDECHAR;  start: CARDINAL) =
  ...
  BEGIN
    ...
    IF (t_offset + t.a_len > start) THEN
      u_offset := MAX (start - t_offset, 0);
      TextClass.SaveRecurseDepth (TextClass.Op.get_wide_chars, RDepth); 
      TextClass.NoteRecurse (TextClass.Op.get_wide_chars); 
      u.get_wide_chars 
        (SUBARRAY (a, a_offset, NUMBER (a) - a_offset), u_offset);
      TextClass.RestoreRecurseDepth (TextClass.Op.get_wide_chars, RDepth); 
      INC (a_offset, t.a_len - u_offset);
      IF (a_offset >= NUMBER (a)) THEN RETURN; END;
    END;
    INC (t_offset, t.a_len);
      ...
      THEN (* The b-side is empty, for us. *) 
        ...
        THEN
          ...
          THEN 
            (* Dispatch and Tail-recursion elimination of the code in the
               ELSE clause below. *) 
            TextClass.NoteIter (TextClass.Op.get_wide_chars); 
            Text := Text.a;
            Chars_charCt := a_charCt;
            start := a_start
            (* And loop. *)  
