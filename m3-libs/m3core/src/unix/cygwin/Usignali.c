...
"  SIGILL    =  4;      (* illegal instruction (not reset when caught) *)", SIGILL,
"  SIGTRAP   =  5;      (* trace trap (not reset when caught) *)", SIGTRAP,
"  SIGIOT    =  6;      (* IOT instruction *)", 6,
"  (* Linux 1.1.73 doesn't have SIGEMT - rrw *)", 0,
"  SIGEMT    =  7;      (* EMT instruction *)", 7,
"  SIGBUS    =  7;      (* bus error *)", 7,
...
"  SIGUSR2   =  %u;     (* User signal 2 (from SysV) *)", 12,
...
"  SIGSTKFLT =  %u;", 16,
"  SIGCHLD   =  %u;     (* to parent on child stop or exit *)", 17,
"  SIGCONT   =  %u;     (* continue a stopped process *)", 18,
"  SIGSTOP   =  %u;     (* sendable stop signal not from tty *)", 19,
"  SIGTSTP   =  %u;     (* stop signal from tty *)", 20,
...
"  SIG_BLOCK    = %u;    (* Add these signals to block mask *)", 0,
"  SIG_UNBLOCK  = %u;    (* Remove these signals from block mask *)", 1,
"  SIG_SETMASK  = %u;    (* Set block mask to this mask *)", 2,
"", 0,
"TYPE", 0,
