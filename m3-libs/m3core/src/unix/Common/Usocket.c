...
/*
SOCKLEN_T is equivalent to socklen_t, when socklen_t is 32 bits.
...
int Usocket__bind(int s, sockaddr_t* name, SOCKLEN_T len)
{
    ASSERT_LEN
    return bind(s, name, len);
}

int Usocket__connect(int s, sockaddr_t* name, SOCKLEN_T len)
{
    ASSERT_LEN
    return connect(s, name, len);
}

int Usocket__sendto(int s, void* msg, size_t length, int flags, sockaddr_t* dest, SOCKLEN_T len)
{
    ASSERT_LEN
    return sendto(s, msg, length, flags, dest, len);
}

int Usocket__setsockopt(int s, int level, int optname, void* optval, SOCKLEN_T len)
{
    ASSERT_LEN
...
int Usocket__getpeername(int s, sockaddr_t* name, SOCKLEN_T* plen)
{
    ASSERT_PLEN
    return getpeername(s, name, plen);
}

int Usocket__getsockname(int s, sockaddr_t* name, SOCKLEN_T* plen)
{
    ASSERT_PLEN
    return getsockname(s, name, plen);
}

int Usocket__accept(int s, sockaddr_t* addr, SOCKLEN_T* plen)
{
    ASSERT_PLEN
    return accept(s, addr, plen);
}

int Usocket__getsockopt(int s, int level, int optname, void* optval, SOCKLEN_T* plen)
{
    ASSERT_PLEN
    return getsockopt(s, level, optname, optval, plen);
}

int Usocket__recvfrom(int s, void* buf, size_t len, int flags, sockaddr_t* from, SOCKLEN_T* plen)
{
    ASSERT_PLEN
...
int Usocket__recvfrom(int s, void* buf, size_t length, int flags, sockaddr_t* address, SOCKLEN_T* plen)
{
    ASSERT_PLEN
    {
        socklen_t len = plen ? *plen : 0;
