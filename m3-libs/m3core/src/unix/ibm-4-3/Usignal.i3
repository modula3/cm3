...
(* Last modified on Mon Mar 16 18:04:03 PST 1992 by muller        *)
...
CONST
  ...
  SIGRETRACT = SIGEMT;		(* retract acces to the screen *)
  SIGGRANT   = SIGUSR2;		(* replace access to the screen *)
...
CONST
    MAXSIGREGS = 27; (* (EX2_RSV-R0+1) number of regs passed to  sighandler *)
TYPE
    struct_sigcontext = RECORD
      sc_flags : int;	(* state to restore *)
      sc_mask : int;	(* signal mask to restore *)
      sc_icscs : int;	(* Must be before sc_regs! *)
      sc_regs : ARRAY [0..MAXSIGREGS - 1] OF INTEGER;   (* saved registers *)
      sc_saveiar : int;   (* saved copy of IAR for RTFL *)
      sc_floatsave : ADDRESS; (* SC_FLOATSAVE -> struct floatsave * *)
    END;
