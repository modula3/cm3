...
(* Last modified on Sat Nov 22 20:02:06 PST 1997 by heydon *)
(* Copyright (C) 1997, Digital Equipment Corporation       *)

MODULE SLTest EXPORTS Main;

(* This program is a simple test of the skip list sorted table
   implementation.

   SYNTAX: SLTest [ numEntries [ randTrials ] ]
*)

IMPORT SkipListIntIntTbl;
IMPORT FloatMode, Lex, Params, Random, Scan, Thread, Wr;
FROM Stdio IMPORT stderr;

<* FATAL Wr.Failure, Thread.Alerted *>

CONST UseFixedSeed = TRUE;
...
TYPE
  T = SkipListIntIntTbl.T;
  Iter = SkipListIntIntTbl.Iterator;
...
PROCEDURE TestRandom(tbl: T) =
  BEGIN
    FOR i := 0 TO RandTrials - 1DO
      EVAL tbl.init(maxSizeHint := NumEntries);
      InitRandom(i);
...
BEGIN
  TRY
    ...
    END;
    (* A skip list is a randomized data structure, so do each
       trial multiple times. *)
    VAR t := NEW(T); BEGIN
      FOR i := 1 TO RandTrials DO TestTbl(t) END
    END
  EXCEPT
