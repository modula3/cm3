...
PROCEDURE PlanDFT1D (in, out: REF ARRAY OF Complex;
  ...
  BEGIN
    ...
    plan^ := RealFFTWRaw.PlanDFT1D(n, in[0], out[0], dirToSign[sign],
...
PROCEDURE PlanDFT2D (in, out: REF ARRAY OF ARRAY OF Complex;
  ...
  BEGIN
    ...
    END;
    plan^ :=
      RealFFTWRaw.PlanDFT2D(nx, ny, in[0, 0], out[0, 0], dirToSign[sign],
                            LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanDFT2D;
...
PROCEDURE PlanDFTR2C2D (in : REF ARRAY OF ARRAY OF R.T;
  ...
  BEGIN
    IF nx # NUMBER(out^) OR ny # NUMBER(out[0]) THEN
      RAISE SizeMismatch;
    END;
    plan^ := RealFFTWRaw.PlanDFTR2C2D(nx, ny, in[0, 0], out[0, 0],
                                      LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanDFTR2C2D;
...
PROCEDURE PlanDFTR2C3D (in : REF ARRAY OF ARRAY OF ARRAY OF R.T;
  ...
  BEGIN
    ...
    END;
    plan^ :=
      RealFFTWRaw.PlanDFTR2C3D(nx, ny, nz, in[0, 0, 0], out[0, 0, 0],
                               LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanDFTR2C3D;
...
PROCEDURE PlanDFTC2R2D (in : REF ARRAY OF ARRAY OF Complex;
  ...
  BEGIN
    IF nx # NUMBER(out^) OR ny # NUMBER(out[0]) DIV 2 + 1 THEN
      RAISE SizeMismatch;
    END;
    plan^ := RealFFTWRaw.PlanDFTC2R2D(nx, ny, in[0, 0], out[0, 0],
                                      LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanDFTC2R2D;
...
PROCEDURE PlanDFTC2R3D (in : REF ARRAY OF ARRAY OF ARRAY OF Complex;
  ...
  BEGIN
    ...
    END;
    plan^ :=
      RealFFTWRaw.PlanDFTC2R3D(nx, ny, nz, in[0, 0, 0], out[0, 0, 0],
                               LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanDFTC2R3D;
...
PROCEDURE PlanR2R1D (in, out: REF ARRAY OF R.T;
  ...
  BEGIN
    IF n # NUMBER(out^) DIV 2 + 1 THEN RAISE SizeMismatch; END;
    plan^ := RealFFTWRaw.PlanR2R1D(n, in[0], out[0], ORD(kind),
                                   LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanR2R1D;
...
PROCEDURE PlanR2R2D (in, out     : REF ARRAY OF ARRAY OF R.T;
  ...
  BEGIN
    ...
    END;
    plan^ :=
      RealFFTWRaw.PlanR2R2D(nx, ny, in[0, 0], out[0, 0], ORD(kindx),
                            ORD(kindy), LOOPHOLE(flags, C.unsigned_int));
    EVAL WeakRef.FromRef(plan, CleanupPlan);
    RETURN plan;
  END PlanR2R2D;
...
PROCEDURE DestroyPlan (p: Plan; ) =
  BEGIN
    RealFFTWRaw.DestroyPlan(p^);
  END DestroyPlan;

PROCEDURE ForgetWisdom () =
  BEGIN
    RealFFTWRaw.ForgetWisdom();
  END ForgetWisdom;
...
PROCEDURE ExportWisdomToFile (output_file: Cstdio.FILE_star; ) =
  BEGIN
    RealFFTWRaw.ExportWisdomToFile(output_file);
  END ExportWisdomToFile;

PROCEDURE ExportWisdomToString (): TEXT =
  VAR result: C.char_star;
  BEGIN
    result := RealFFTWRaw.ExportWisdomToString();
...
PROCEDURE ExportWisdom (VAR write_char: PROCEDURE (c: CHAR; buf: ADDRESS; );
                        VAR data: REFANY; ) =
  BEGIN
    RealFFTWRaw.ExportWisdom(write_char, LOOPHOLE(data, ADDRESS));
  END ExportWisdom;

PROCEDURE ImportSystemWisdom (): INTEGER =
  BEGIN
    RETURN RealFFTWRaw.ImportSystemWisdom();
  END ImportSystemWisdom;

PROCEDURE ImportWisdomFromFile (input_file: Cstdio.FILE_star; ): INTEGER =
  BEGIN
    RETURN RealFFTWRaw.ImportWisdomFromFile(input_file);
...
PROCEDURE ImportWisdomFromString (input_string: TEXT; ): INTEGER =
  ...
  BEGIN
    ...
    result := RealFFTWRaw.ImportWisdomFromString(arg1);
...
PROCEDURE ImportWisdom (VAR read_char: PROCEDURE (buf: ADDRESS; ): CARDINAL;
  ...
  BEGIN
    RETURN RealFFTWRaw.ImportWisdom(read_char, LOOPHOLE(data, ADDRESS));
  END ImportWisdom;

PROCEDURE FPrintPlan (p: Plan; output_file: Cstdio.FILE_star; ) =
  BEGIN
    RealFFTWRaw.FPrintPlan(p^, output_file);
  END FPrintPlan;

PROCEDURE PrintPlan (p: Plan; ) =
  BEGIN
    RealFFTWRaw.PrintPlan(p^);
  END PrintPlan;

PROCEDURE Flops (p: Plan; VAR add, mul, fma: LONGREAL; ) =
  BEGIN
    RealFFTWRaw.Flops(p^, add, mul, fma);
  END Flops;
