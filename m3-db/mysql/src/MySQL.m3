...
PROCEDURE NumRows (res: ResultT; ): LONGINT =
  VAR arg1: MySQLRaw.MYSQL_RES := LOOPHOLE(res, MySQLRaw.MYSQL_RES);
  BEGIN
...
PROCEDURE Eof (res: ResultT; ): BOOLEAN =
  VAR
    ret   : MySQLRaw.my_bool;
    result: BOOLEAN;
    arg1  : MySQLRaw.MYSQL_RES := LOOPHOLE(res, MySQLRaw.MYSQL_RES);
  BEGIN
...
PROCEDURE FieldCount (mysql: T; ): CARDINAL =
  VAR arg1: MySQLRaw.MYSQL := LOOPHOLE(mysql, MySQLRaw.MYSQL);
  BEGIN
...
PROCEDURE Error (mysql: T; ): TEXT =
  VAR
    result: C.char_star;
    arg1  : MySQLRaw.MYSQL := LOOPHOLE(mysql, MySQLRaw.MYSQL);
  BEGIN
...
PROCEDURE GetCharacterSetInfo (mysql: T; VAR charset: CharsT; ) =
  VAR
    arg1: MySQLRaw.MYSQL := LOOPHOLE(mysql, MySQLRaw.MYSQL);
    arg2: MySQLRaw.MYSQL_CHARSET_INFO := LOOPHOLE(
                                           charset,
                                           MySQLRaw.MYSQL_CHARSET_INFO);
  BEGIN
    MySQLRaw.GetCharacterSetInfo(arg1, arg2);
...
PROCEDURE RowSeek (res: ResultT; offset: RowOffsetT; ): RowOffsetT =
  VAR
    ...
    result: RowOffsetT;
    arg1  : MySQLRaw.MYSQL_RES        := LOOPHOLE(res, MySQLRaw.MYSQL_RES);
    arg2: MySQLRaw.MYSQL_ROW_OFFSET := LOOPHOLE(
                                         offset, MySQLRaw.MYSQL_ROW_OFFSET);
  BEGIN
...
PROCEDURE StmtPrepare (stmt: StmtT; query: TEXT; length: CARDINAL; ):
  ...
  VAR
    arg1  : MySQLRaw.MYSQL_STMT := LOOPHOLE(stmt, MySQLRaw.MYSQL_STMT);
    arg2  : C.const_char_star;
    result: INTEGER;
  BEGIN
    TRY
...
PROCEDURE StmtFetchColumn
  ...
  VAR
    arg1  : MySQLRaw.MYSQL_STMT := LOOPHOLE(stmt, MySQLRaw.MYSQL_STMT);
    arg2  : MySQLRaw.MYSQL_BIND := LOOPHOLE(bind, MySQLRaw.MYSQL_BIND);
    result: INTEGER;
  BEGIN
...
PROCEDURE StmtParamCount (stmt: StmtT; ): CARDINAL =
  VAR arg1: MySQLRaw.MYSQL_STMT := LOOPHOLE(stmt, MySQLRaw.MYSQL_STMT);
  BEGIN
...
PROCEDURE StmtBindParam (stmt: StmtT; bnd: BindT; ): BOOLEAN =
  VAR
    ...
    result: BOOLEAN;
    arg1  : MySQLRaw.MYSQL_STMT := LOOPHOLE(stmt, MySQLRaw.MYSQL_STMT);
    arg2  : MySQLRaw.MYSQL_BIND := LOOPHOLE(bnd, MySQLRaw.MYSQL_BIND);
  BEGIN
...
PROCEDURE Close (sock: T; ) =
  VAR arg1: MySQLRaw.MYSQL := LOOPHOLE(sock, MySQLRaw.MYSQL);
  BEGIN
    MySQLRaw.Close(arg1);
  END Close;
