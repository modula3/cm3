...
MODULE TrackerPositionSO EXPORTS TrackerPosition, TrackerPositionProxy;

IMPORT TrackerPositionCB, TrackerPositionF;
IMPORT EventStubLib, EventProtocol, Rd, Wr, SharedObj, SharedObjRep,
       SharedObjStubLib, SharedObjError, Thread, ThreadF, Event, Pickle,
       WeakRef, ObjectSpace, IO;
IMPORT Time, Fmt;
...
PROCEDURE Shared_get (self: T): Data
...
PROCEDURE Shared_set (self   : T; READONLY val_arg: Data)
  ...
  BEGIN
    IF NOT self.ok THEN SharedObjError.RaiseDeadObject(); END;
    IF self.updating = id THEN SharedObjError.RaiseRecursiveUpdate(); END;
    TRY
      m := SharedObjStubLib.StartCall(self);
      IF SharedObjStubLib.MarshalArgs(m) THEN
        WITH ts = Time.Now() DO
          TRY
            EventStubLib.OutInt32(m.eh, ORD(Methods.set));
            SharedObjStubLib.OutRef(m.eh, val_arg);
          FINALLY
            WITH diff = Time.Now() - ts DO
              IF diff > 0.1D0 THEN
                IO.Put("Marshalling took too long: " & Fmt.LongReal(diff) &
                  "\n");
              END;
            END;
          END;
...
*)
      END;
      SharedObjStubLib.SequenceCall(m, Protocol);
      WITH ts = Time.Now() DO
      TRY
      ...
      FINALLY
        ...
        WITH diff = Time.Now() - ts DO
          IF diff > 0.1D0 THEN
            IO.Put("applying took too long: " & Fmt.LongReal(diff) &
              "\n");
          END;
        END;
      END;
...
PROCEDURE Stub_set (self: T; h: EventStubLib.Handle)
  ...
  VAR val_arg: Data;
  BEGIN
    val_arg := SharedObjStubLib.InRef(h, TYPECODE(Data));
(*
...
*)
    IF tc = TYPECODE(T) THEN
      ...
      IF MkProxyT # NIL AND obj.proxy = NIL THEN
        MkProxyT (obj);
      END;
    END;
