GENERIC MODULE M3CBackEnd_Int_Longint_xg(IntType, WordType);
...
VAR
  small_of_M3CBackEnd_Int_Longint_xg := ARRAY [0..255] OF T {NIL, ..};
  first := NEW(T, sm_value := FIRST(IntType.T));
  last := NEW(T, sm_value := LAST(IntType.T));

PROCEDURE New_value(i: IntType.T): T RAISES {} =
  BEGIN
    IF VAL(FIRST(small_of_M3CBackEnd_Int_Longint_xg), IntType.T) <= i
      AND i <= VAL(LAST(small_of_M3CBackEnd_Int_Longint_xg), IntType.T) THEN
      WITH result = small_of_M3CBackEnd_Int_Longint_xg[VAL(i, INTEGER)] DO
