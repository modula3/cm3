...
(* Last modified on Mon Jan 30 16:09:03 PST 1995 by kalsow                   *)
...
IMPORT Text, Wx, Thread, Scheduler;

TYPE 
  (* RefStream = ARRAY [0..LAST(CARDINAL)] OF REFANY; *)

REVEAL
  T = T_ BRANDED "XFormat.T" OBJECT
    wx:          Wx.T;    
    ...
    closed:      BOOLEAN;          (* /Close/ operator is through the pipe *)
...
PROCEDURE New(wx: Wx.T;  width: CARDINAL:= 75): T =
  BEGIN
    WITH t = NEW(T) DO
      t.wx := wx;
      t.width := width;
...
PROCEDURE UnderlyingWr(t: T): Wx.T =
  BEGIN
    RETURN t.wx;
  END UnderlyingWr;
...
PROCEDURE PutText(t: T;  text: Text.T;  raw := FALSE) =
  BEGIN
    IF raw THEN
      IF t.numChars > 0 THEN AddChars(t) END;
      AddRef(t, text);
...
PROCEDURE PutMarkup(t: T;  text: Text.T;  width := 0) =
  BEGIN
    IF t.numChars > 0 THEN AddChars(t) END;
    AddRef(t, MarkUpOp);
    AddRef(t, text);
...
PROCEDURE AddRef(t: T; ref: REFANY) =
  (*
    Adds an element to the Producer buffer.  If the buffer gets big
    enough, or the added element is the "Flush" operator, then releases
...
PROCEDURE Allocate(
  ...
  BEGIN
    LOCK t.lock DO
      IF t.size[this] < minSize THEN
        INC (t.waiters);
        WHILE (t.size[this] < minSize) DO
...
<*INLINE*> 
PROCEDURE Probe(t: T; i: CARDINAL) =
  (*
...
<*INLINE*> 
PROCEDURE Get(t: T; i: CARDINAL): REFANY =
  BEGIN
    RETURN t.buffer[i MOD t.bufSize];
  END Get;

<*INLINE*> 
PROCEDURE GetB (t: T; i: CARDINAL): BOOLEAN =
  BEGIN
    RETURN NARROW (t.buffer[i MOD t.bufSize], Bool)^;
  END GetB;

<*INLINE*> 
PROCEDURE GetI (t: T; i: CARDINAL): INTEGER =
  BEGIN
    RETURN NARROW (t.buffer[i MOD t.bufSize], Int)^;
  END GetI;

<*INLINE*> 
PROCEDURE GetBreakType (t: T; i: CARDINAL): BreakType =
  BEGIN
...
PROCEDURE PrintTop(self: ConsumerThreadClosure): REFANY =
  VAR pos: Position;
  (*
    The printing thread *)
  BEGIN
...
PROCEDURE DoPrintText(
  ...
  BEGIN
    ...
    IF mode = Mode.Writing THEN Wx.PutText(t.wx, text) END;
    pos.c := pos.c + Text.Length(text);
    RETURN (mode = Mode.Thinking) AND (pos.c > t.width);
  END DoPrintText;

PROCEDURE PrintMarkUp(
    t: T;
    mode: Mode;
    VAR pos: Position;
    <*UNUSED*> maxL: CARDINAL;
  ...
  (*
    Prints a zero-width, markup string. *)
  BEGIN
    RETURN DoPrintMarkUp(t, mode, pos, Get(t, args), GetI(t, args+1));
  END PrintMarkUp;

PROCEDURE DoPrintMarkUp(
    t: T;
    mode: Mode;
    VAR pos: Position;
    text: Text.T;
...
PROCEDURE DoPrintChar(
  ...
  BEGIN
    ...
    ELSE
      IF pos.b > 0 THEN DoTrailingBlanks(t, mode, pos) END;
      IF mode = Mode.Writing THEN Wx.PutChar(t.wx, ch^) END;
      pos.c := pos.c + 1;
      RETURN (mode = Mode.Thinking) AND (pos.c > t.width);
    END;
  END DoPrintChar;
...
PROCEDURE DoTrailingBlanks(
  ...
  BEGIN
    IF mode = Mode.Writing THEN
      FOR i := 1 TO pos.b DO Wx.PutChar(t.wx, ' ') END;
    END;
    pos.c := pos.c + pos.b;
    pos.b := 0;
...
PROCEDURE DoNewLine(
  ...
  BEGIN
    IF mode = Mode.Writing THEN Wx.PutChar(t.wx, '\n') END;
    pos.c := 0;
...
PROCEDURE ClearWidths(widths: Widths) =
  BEGIN
...
BEGIN
  ...
  MarkUpOp       := DefineOp(PrintMarkUp,       2, 7);
  ...
  CloseOp        := DefineOp(PrintClose,        0, FIRST (INTEGER) + 1);
